...
public static class Test {
    ...
    public static void Success<T>(Result<T> result, Func<T, bool>? predicate = null, string? message = null) =>
        result.Match(
            onSuccess: value => predicate is null || predicate(value) ? true : throw new InvalidOperationException(message ?? string.Create(CultureInfo.InvariantCulture, $"Success predicate failed for value {value}")),
    ...
    public static void Failure<T>(Result<T> result, Func<SystemError[], bool>? predicate = null, string? message = null) =>
        result.Match(
            ...
            onFailure: errors => predicate is null || predicate(errors) ? true : throw new InvalidOperationException(message ?? string.Create(CultureInfo.InvariantCulture, $"Failure predicate violated for errors: {string.Join(", ", errors.Select(e => e.Message))}")));
