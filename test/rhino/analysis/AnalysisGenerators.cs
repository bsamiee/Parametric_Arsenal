...
public static class AnalysisGenerators {
    ...
    [Pure] public static Gen<Sphere> SphereGen =>
    ...
    [Pure] public static Gen<NurbsSurface> SphereSurfaceGen =>
    ...
    [Pure] public static Gen<Mesh> SphereMeshGen =>
    ...
    /// <summary>Generates valid meshes from Breps (non-mutating).</summary>
    [Pure] public static Gen<Mesh> BrepMeshGen =>
        BrepGen.Select(static brep => {
    ...
    private static Mesh? CombineMeshes(Mesh[]? meshes) {
        ...
        _ = combined is null || meshes is null || (combined.Append(meshes) is var _ && true);
        return combined;
    }
    ...
    [Pure] public static Gen<int> DerivativeOrderGen => Gen.Int[1, 3];

    /// <summary>Generates normalized parameter values in [0.1,0.9] avoiding boundary singularities.</summary>
    [Pure] public static Gen<double> NormalizedParameterGen => Gen.Double[0.1, 0.9];

    /// <summary>Generates normalized UV parameter pairs in [0.1,0.9] x [0.1,0.9].</summary>
    [Pure] public static Gen<(double U, double V)> UVParameterGen =>
        NormalizedParameterGen.SelectMany(u =>
            NormalizedParameterGen.Select(v => (u, v)));

    /// <summary>Generates multiple normalized parameters for batch testing.</summary>
    [Pure] public static Gen<IReadOnlyList<double>> MultipleParametersGen =>
        Gen.Int[3, 5].SelectMany(count =>
            NormalizedParameterGen.List[count, count].Select(static list => (IReadOnlyList<double>)list));

    /// <summary>Generates multiple UV parameter pairs for batch testing.</summary>
    [Pure] public static Gen<IReadOnlyList<(double U, double V)>> MultipleUVParametersGen =>
        Gen.Int[2, 4].SelectMany(count =>
            UVParameterGen.List[count, count].Select(static list => (IReadOnlyList<(double U, double V)>)list));
}
