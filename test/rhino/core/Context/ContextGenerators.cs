...
public static class ContextGenerators {
    ...
    [Pure] public static Gen<double> ValidToleranceGen => Gen.Double[1e-9, 1e3].Where(static d => RhinoMath.IsValidDouble(d) && d > RhinoMath.ZeroTolerance);

    /// <summary>Generates valid relative tolerance values (0 to 1, exclusive).</summary>
    [Pure] public static Gen<double> ValidRelativeToleranceGen => Gen.Double[0.0, 0.999].Where(static d => RhinoMath.IsValidDouble(d) && d >= 0.0 && d < 1.0);

    /// <summary>Generates valid angle tolerance values in radians.</summary>
    [Pure] public static Gen<double> ValidAngleToleranceGen => Gen.Double[RhinoMath.Epsilon, RhinoMath.TwoPI].Where(static d => RhinoMath.IsValidDouble(d));

    /// <summary>Generates all Rhino unit systems with uniform distribution.</summary>
    [Pure] public static Gen<UnitSystem> UnitSystemGen => Gen.OneOfConst(AllUnitSystems);

    /// <summary>Generates common unit systems (higher probability for typical use cases).</summary>
    [Pure] public static Gen<UnitSystem> CommonUnitSystemGen => Gen.OneOfConst(CommonUnitSystems);

    /// <summary>Generates valid GeometryContext via Create() method with property-based tolerances.</summary>
    [Pure] public static Gen<GeometryContext> GeometryContextGen =>
        ValidToleranceGen
            .SelectMany(absTol =>
    ...
    [Pure] public static Gen<GeometryContext> DefaultContextGen =>
        CommonUnitSystemGen
    ...
    [Pure] public static Gen<GeometryContext> HighPrecisionContextGen =>
    ...
    [Pure] public static Gen<GeometryContext> LowPrecisionContextGen =>
        Gen.Double[0.1, 10.0]
            .Where(static d => RhinoMath.IsValidDouble(d))
            .SelectMany(tol =>
                CommonUnitSystemGen.Select(units =>
