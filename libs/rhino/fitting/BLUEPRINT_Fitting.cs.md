# Blueprint: Fitting.cs (Main Public API)

## File Purpose
Main public API entry point with nested result types, unified dispatch, and suppression attribute for namespace matching.

## Total Types: 6 (all nested within Fitting class)
- `Fitting` (static class) - Main API
- `IFitResult` (interface) - Polymorphic result marker
- `CurveFitResult` (sealed record) - Curve fitting result
- `SurfaceFitResult` (sealed record) - Surface fitting result  
- `FitOptions` (readonly record struct) - Fitting configuration
- `FairOptions` (readonly record struct) - Fairing configuration
...
## Key Patterns
- All types nested in main class (no separate files)
- Suppression attribute for namespace match
- XML docs: one-line summary only
- Named parameters throughout
- Target-typed new() for OperationConfig
- Pure + AggressiveInlining on all public methods
- Tuple parameters for multi-value options
- DebuggerDisplay with interpolated strings
