# Blueprint: FittingCore.cs (Core Algorithms)

## File Purpose
Dense algorithmic implementation for chord-length parameterization, knot vector generation, least-squares solving, Cox-de Boor basis functions, and quality metrics computation.

## Total Types: 1
- `FittingCore` (internal static class)

## Estimated LOC: 270-290
...
## Key Patterns
- Dense algorithmic code (270 LOC)
- Cox-de Boor recursion inline (no helper extraction)
- Chord-length formula: u[i] = Σ||p[j]-p[j-1]||^power / totalLength
- Knot averaging: knot[i] = (t[i-p] + ... + t[i-1]) / p
- Gaussian elimination with pivoting for least-squares
- RhinoMath constants throughout
- ArrayPool for temporary buffers
- All validations via pattern matching
