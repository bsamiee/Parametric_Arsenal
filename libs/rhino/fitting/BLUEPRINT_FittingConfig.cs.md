# Blueprint: FittingConfig.cs (Constants & Dispatch)

## File Purpose
Single FrozenDictionary dispatch table with enum-based operations, constants, validation mode mappings.

## Total Types: 2
- `FittingConfig` (internal static class) - Configuration and dispatch
- `FitOperation` (internal enum) - Operation type dispatch keys

## Estimated LOC: 90-110
...
## Key Patterns
- Single FrozenDictionary with enum keys (no unnecessary bytes)
- All constants use RhinoMath where appropriate
- Inline method for computed constraint
- Pure attribute on class
- Trailing commas in dictionary
- No validation mode duplication across entries
