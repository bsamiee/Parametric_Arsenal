    ...
    private static Result<T> ExecuteToPlane<T>(T geometry, Plane target, OrientationConfig.OrientationOperationMetadata meta, IGeometryContext context) where T : GeometryBase =>
        UnifiedOperation.Apply(
            ...
            operation: (Func<T, Result<IReadOnlyList<T>>>)(item =>
                (OrientationConfig.PlaneExtractors.TryGetValue(item.GetType(), out OrientationConfig.PlaneExtractorMetadata? extractor)
                    ? extractor.Extractor(item)
                    : OrientationConfig.PlaneExtractors.FirstOrDefault(kv => kv.Key.IsInstanceOfType(item)).Value?.Extractor.Invoke(item)
