...
internal static class OrientCompute {
    ...
    internal static Result<(Transform OptimalTransform, double Score, byte[] CriteriaMet)> OptimizeOrientation(
        ...
        brep.GetBoundingBox(accurate: true) is BoundingBox box && box.IsValid
            ? (brep.IsSolid ? VolumeMassProperties.Compute(brep) : null, [
            ...
            ]) is (VolumeMassProperties? vmp, Plane[] testPlanes)
                ? testPlanes.Select(plane => Transform.PlaneToPlane(plane, Plane.WorldXY) is Transform xf
                    ? (Brep)brep.Duplicate() is Brep test && test.Transform(xf) is bool
                        ? test.GetBoundingBox(accurate: true) is BoundingBox testBox
                            ? (Xform: xf, Score: criteria switch {
                                ...
                                _ => 0.0,
                            }, Met: criteria switch { 1 => [1,], 2 => [2,], 3 => [3,], 4 => [4,], _ => [] }, Test: test)
                            : (Transform.Identity, 0.0, [], test)
                        : (Transform.Identity, 0.0, [], test)
                    : (Transform.Identity, 0.0, [], null)).Select(r => (r.Test?.Dispose(), r) is (ValueTuple?, (Transform, double, byte[], Brep?)) ? (r.Xform, r.Score, r.Met) : (r.Xform, r.Score, r.Met)).OrderByDescending(r => r.Score).First() is (Transform best, double bestScore, byte[] met)
                    ? (vmp?.Dispose(), ResultFactory.Create(value: (best, bestScore, met))) is (ValueTuple?, Result<(Transform, double, byte[])> result) ? result : result
        ...
        geometries.Length >= OrientConfig.PatternMinInstances
            ? geometries.Select(g => OrientCore.ExtractCentroid(g, useMassProperties: false)).Where(r => r.IsSuccess).Select(r => r.Value).ToArray() is Point3d[] centroids && centroids.Length >= 3
                ? centroids.Skip(1).Zip(centroids, (c2, c1) => c2 - c1).ToArray() is Vector3d[] deltas && deltas.Average(v => v.Length) is double avgLen && deltas.All(v => Math.Abs(v.Length - avgLen) / avgLen < tolerance)
                    ? ResultFactory.Create(value: ((byte)0, centroids.Select((_, i) => Transform.Translation(deltas[0] * i)).ToArray(), [], deltas.Select(v => Math.Abs(v.Length - avgLen)).Average()))
                    : new Point3d(centroids.Average(p => p.X), centroids.Average(p => p.Y), centroids.Average(p => p.Z)) is Point3d center && centroids.Select(p => p.DistanceTo(center)).ToArray() is double[] radii && radii.Average() is double avgRadius && radii.All(r => Math.Abs(r - avgRadius) / avgRadius < tolerance)
                        ? ResultFactory.Create(value: ((byte)1, Enumerable.Range(0, centroids.Length).Select(i => Transform.Rotation(2.0 * Math.PI * i / centroids.Length, Vector3d.ZAxis, center)).ToArray(), [], radii.Select(r => Math.Abs(r - avgRadius)).Average()))
    ...
    private static bool TestReflectionSymmetry(Brep a, Brep b, Plane mirror, double tolerance) =>
        a.Vertices.Count == b.Vertices.Count && a.Vertices.Zip(b.Vertices, (va, vb) => va.Location is Point3d reflected && reflected.Transform(Transform.Mirror(mirror)) is bool ? reflected.DistanceTo(vb.Location) < tolerance : false).All(match => match);

    private static bool TestRotationSymmetry(Curve a, Curve b, Point3d center, Vector3d axis, double tolerance) =>
        a.SpanCount == b.SpanCount && Enumerable.Range(0, 8).All(i => a.Domain.ParameterAt(i / 7.0) is double t
            ? a.PointAt(t) is Point3d ptA && b.PointAt(t) is Point3d ptB && Math.Atan2((ptA - center).Y, (ptA - center).X) is double angleA && Math.Atan2((ptB - center).Y, (ptB - center).X) is double angleB
                ? Math.Abs(angleA - angleB) < tolerance
                : false
            : false);
}
