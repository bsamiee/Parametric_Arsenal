...
internal static class OrientationCompute {
    ...
    internal static Result<Plane> ExtractCurvePlane(Curve curve) =>
        curve.FrameAt(curve.Domain.Mid, out Plane frame) && frame.IsValid
            ? ResultFactory.Create(value: frame)
            : ResultFactory.Create<Plane>(error: E.Geometry.FrameExtractionFailed);

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal static Result<Plane> ExtractSurfacePlane(Surface surface) =>
        surface.FrameAt(surface.Domain(0).Mid, surface.Domain(1).Mid, out Plane frame) && frame.IsValid
            ? ResultFactory.Create(value: frame)
            : ResultFactory.Create<Plane>(error: E.Geometry.FrameExtractionFailed);

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal static Result<Plane> ExtractBrepPlane(Brep brep) =>
        brep.IsSolid
            ? ((Func<Result<Plane>>)(() => {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal static Result<Plane> ExtractExtrusionPlane(Extrusion extrusion) =>
        extrusion.IsSolid
            ? ((Func<Result<Plane>>)(() => {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal static Result<Plane> ExtractMeshPlane(Mesh mesh) =>
        mesh.IsClosed
            ? ((Func<Result<Plane>>)(() => {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal static Result<Plane> ExtractPointCloudPlane(PointCloud cloud) =>
        cloud.Count > 0
    ...
    internal static Result<Orientation.OptimizationResult> OptimizeOrientation(Brep brep, Orientation.OptimizationCriteria criteria, double tolerance) =>
        tolerance <= 0.0
            ...
            : brep.GetBoundingBox(accurate: true) is not BoundingBox box || !box.IsValid
    ...
    internal static Result<Orientation.RelativeOrientationResult> ComputeRelative(GeometryBase geometryA, GeometryBase geometryB, double symmetryTolerance, double angleTolerance) =>
        (OrientationConfig.PlaneExtractors.TryGetValue(geometryA.GetType(), out Func<GeometryBase, Result<Plane>>? extA),
         OrientationConfig.PlaneExtractors.TryGetValue(geometryB.GetType(), out Func<GeometryBase, Result<Plane>>? extB)) switch {
            (true, true) when extA!(geometryA) is Result<Plane> ra && extB!(geometryB) is Result<Plane> rb =>
                (ra, rb) switch {
            ...
            (Brep ba, Brep bb) when ba.Vertices.Count == bb.Vertices.Count =>
                (pb.Origin - pa.Origin).Length > RhinoMath.ZeroTolerance
                    ? new Plane(pa.Origin + ((pb.Origin - pa.Origin) * 0.5), pb.Origin - pa.Origin) is Plane mirror && mirror.IsValid
                      && ba.Vertices.Select(va => { Point3d r = va.Location; r.Transform(Transform.Mirror(mirrorPlane: mirror)); return r; }).ToArray() is Point3d[] reflectedA
                    ...
                    : new Plane(pa.Origin, pa.ZAxis) is Plane mirror2 && mirror2.IsValid
                      && ba.Vertices.Select(va => { Point3d r = va.Location; r.Transform(Transform.Mirror(mirrorPlane: mirror2)); return r; }).ToArray() is Point3d[] reflectedA2
                      && reflectedA2.All(ra => bb.Vertices.Any(vb => ra.DistanceTo(vb.Location) < tolerance))
                      && bb.Vertices.All(vb => reflectedA2.Any(ra => ra.DistanceTo(vb.Location) < tolerance))
                        ? new Orientation.MirrorSymmetry()
                        : new Orientation.NoSymmetry(),
                ...
                return centroidResults.All(static r => r.IsSuccess)
                    ? centroidResults.Select(static r => r.Value).ToArray() is Point3d[] centroids && centroids.Length >= 3
                      ...
                      && deltas.Average(static v => v.Length) is double avgLen && avgLen > absoluteTolerance
                        ...
                        : ResultFactory.Create<Orientation.PatternDetectionResult>(error: E.Geometry.PatternDetectionFailed.WithContext("Insufficient valid centroids"))
