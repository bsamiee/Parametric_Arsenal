...
internal static class FieldsCompute {
    ...
    internal static Result<(Point3d[] Grid, double[] Product)> ScalarVectorProduct(
        ...
        component switch {
            Fields.XComponent => ApplyBinaryFieldOperation(
                inputField1: scalarField,
                inputField2: vectorField,
                grid: grid,
                operation: (scalar, vector, _) => scalar * vector.X,
            ...
            Fields.YComponent => ApplyBinaryFieldOperation(
                inputField1: scalarField,
                inputField2: vectorField,
                grid: grid,
                operation: (scalar, vector, _) => scalar * vector.Y,
                error1: E.Geometry.InvalidFieldComposition.WithContext("Scalar field length must match grid points"),
                error2: E.Geometry.InvalidFieldComposition.WithContext("Vector field length must match grid points")),
            Fields.ZComponent => ApplyBinaryFieldOperation(
                inputField1: scalarField,
                inputField2: vectorField,
                grid: grid,
                operation: (scalar, vector, _) => scalar * vector.Z,
                error1: E.Geometry.InvalidFieldComposition.WithContext("Scalar field length must match grid points"),
                error2: E.Geometry.InvalidFieldComposition.WithContext("Vector field length must match grid points")),
            _ => ResultFactory.Create<(Point3d[], double[])>(error: E.Geometry.InvalidFieldComposition.WithContext($"Unsupported component selector: {component.GetType().Name}")),
        };
                        ...
                        for (int k = 1; k < resolution - 1; k++) {
                            ...
                            if (gradientField[idx].Length < FieldsConfig.MinFieldMagnitude) {
                                ...
                                criticalPoints.Add(new Fields.CriticalPoint(
                                    Location: grid[idx],
                                    Classification: (positiveCount, negativeCount) switch {
                                        (3, 0) => Fields.Minimum,
                                        (0, 3) => Fields.Maximum,
                                        _ => Fields.Saddle,
                                    },
