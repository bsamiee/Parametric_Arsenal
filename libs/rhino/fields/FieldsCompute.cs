...
internal static class FieldsCompute {
    ...
    internal static Result<(Point3d[] Grid, Vector3d[] Gradients)> ComputeGradient(
        ...
        try {
            ...
            return ResultFactory.Create<(Point3d[] Grid, Vector3d[] Gradients)>(value: (
                Grid: grid,
        ...
        try {
            for (int seedIdx = 0; seedIdx < seeds.Length; seedIdx++) {
                ...
                for (int step = 0; step < FieldsConfig.MaxStreamlineSteps - 1; step++) {
                    ...
                    bool shouldContinue = delta.Length > tolerance && stepCount < FieldsConfig.MaxStreamlineSteps - 1;
                ...
                streamlines[seedIdx] = stepCount > 1
                    ...
                    : new LineCurve(seeds[seedIdx], seeds[seedIdx] + new Vector3d(tolerance, 0, 0));
            }
    ...
    internal static Result<Mesh[]> ExtractIsosurfaces(
        ...
        for (int isoIdx = 0; isoIdx < isovalues.Length; isoIdx++) {
            ...
            int[] faceIndices = mesh.Faces.AddFaces([.. faces]);
            bool normalsComputed = mesh.Normals.ComputeNormals();
            bool compacted = mesh.Compact();
            meshes[isoIdx] = mesh;
        }
