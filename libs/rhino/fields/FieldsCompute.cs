...
internal static class FieldsCompute {
    ...
    private static Result<T> InterpolateField<T>(
        ...
        int resolution,
        BoundingBox bounds,
        Fields.InterpolationMethod interpolationMethod,
        ...
        interpolationMethod switch {
            ...
            _ => ResultFactory.Create<T>(error: E.Geometry.InvalidFieldInterpolation.WithContext($"Unsupported interpolation method: {interpolationMethod.GetType().Name}")),
        };
                ...
                for (int seedIdx = 0; seedIdx < seeds.Length; seedIdx++) {
                    ...
                    for (int step = 0; step < FieldsConfig.MaxStreamlineSteps - 1; step++) {
                        ...
                        Vector3d delta = integrationMethod switch {
                            ...
                            Fields.RK2Integration => stepSize * Interpolate(FieldsConfig.RK2HalfStep, k1),
                            Fields.RK4Integration => ((Func<Vector3d>)(() => {
                    ...
                    for (int j = 1; j < resolution - 1; j++) {
                        for (int k = 1; k < resolution - 1; k++) {
                            ...
                            if (gradientField[idx].Length < FieldsConfig.MinFieldMagnitude) {
                                ...
                                Fields.CriticalPointType criticalType = (positiveCount, negativeCount) switch {
                                    (3, 0) => new Fields.Minimum(),
                                    (0, 3) => new Fields.Maximum(),
                                    _ => new Fields.Saddle(),
                                };
