...
internal static class FieldsCore {
    ...
    internal static Result<(Point3d[] Grid, Vector3d[] Gradients)> GradientField(
        ...
        BoundingBox bounds,
        IGeometryContext context) {
        FieldsConfig.FieldOperationMetadata metadata = FieldsConfig.DifferentialOperations[typeof(FieldsConfig.GradientOp)];
        ...
        return UnifiedOperation.Apply(
            ...
            operation: (Func<(Point3d[], double[]), Result<IReadOnlyList<(Point3d[], Vector3d[])>>>)(field =>
                FieldsCompute.ComputeGradient(
                    distances: field.Item2,
                    grid: field.Item1,
                    resolution: sampling.Resolution,
                    gridDelta: gridDelta).Map(static r => (IReadOnlyList<(Point3d[], Vector3d[])>)[r,])),
    ...
    internal static Result<(Point3d[] Grid, Vector3d[] Curl)> CurlField(
        ...
        BoundingBox bounds,
        IGeometryContext context) {
        FieldsConfig.FieldOperationMetadata metadata = FieldsConfig.DifferentialOperations[typeof(FieldsConfig.CurlOp)];
        Vector3d gridDelta = (bounds.Max - bounds.Min) / (sampling.Resolution - 1);
        return UnifiedOperation.Apply(
    ...
    internal static Result<(Point3d[] Grid, double[] Divergence)> DivergenceField(
        ...
        BoundingBox bounds,
        IGeometryContext context) {
        FieldsConfig.FieldOperationMetadata metadata = FieldsConfig.DifferentialOperations[typeof(FieldsConfig.DivergenceOp)];
        Vector3d gridDelta = (bounds.Max - bounds.Min) / (sampling.Resolution - 1);
        return UnifiedOperation.Apply(
            input: (vectorField, gridPoints),
            operation: (Func<(Vector3d[], Point3d[]), Result<IReadOnlyList<(Point3d[], double[])>>>)(field =>
                FieldsCompute.ComputeDivergence(
                    ...
                    grid: field.Item2,
                    resolution: sampling.Resolution,
                    gridDelta: gridDelta).Map(static r => (IReadOnlyList<(Point3d[], double[])>)[r,])),
    ...
    internal static Result<(Point3d[] Grid, double[] Laplacian)> LaplacianField(
        ...
        BoundingBox bounds,
        IGeometryContext context) {
        FieldsConfig.FieldOperationMetadata metadata = FieldsConfig.DifferentialOperations[typeof(FieldsConfig.LaplacianOp)];
        Vector3d gridDelta = (bounds.Max - bounds.Min) / (sampling.Resolution - 1);
        return UnifiedOperation.Apply(
    ...
    internal static Result<(Point3d[] Grid, double[,][] Hessian)> HessianField(
        ...
        BoundingBox bounds,
        IGeometryContext context) {
        FieldsConfig.FieldOperationMetadata metadata = FieldsConfig.DifferentialOperations[typeof(FieldsConfig.HessianOp)];
        Vector3d gridDelta = (bounds.Max - bounds.Min) / (sampling.Resolution - 1);
        return UnifiedOperation.Apply(
            input: (scalarField, gridPoints),
            operation: (Func<(double[], Point3d[]), Result<IReadOnlyList<(Point3d[], double[,][])>>>)(field =>
    ...
    internal static Result<(Point3d[] Grid, Vector3d[] Potential)> VectorPotentialField(
        ...
        BoundingBox bounds,
        IGeometryContext context) {
        FieldsConfig.FieldOperationMetadata metadata = FieldsConfig.DifferentialOperations[typeof(FieldsConfig.VectorPotentialOp)];
        Vector3d gridDelta = (bounds.Max - bounds.Min) / (sampling.Resolution - 1);
        return UnifiedOperation.Apply(
        ...
        UnifiedOperation.Apply(
            ...
            operation: (Func<(Point3d, double[], Point3d[]), Result<IReadOnlyList<double>>>)(data =>
                FieldsCompute.InterpolateScalar(
                    query: data.Item1,
                    scalarField: data.Item2,
                    grid: data.Item3,
                    resolution: sampling.Resolution,
                    bounds: bounds,
                    mode: mode).Map(static r => (IReadOnlyList<double>)[r,])),
        ...
        UnifiedOperation.Apply(
            ...
            operation: (Func<(Point3d, Vector3d[], Point3d[]), Result<IReadOnlyList<Vector3d>>>)(data =>
                FieldsCompute.InterpolateVector(
                    query: data.Item1,
                    vectorField: data.Item2,
                    grid: data.Item3,
                    resolution: sampling.Resolution,
                    bounds: bounds,
                    mode: mode).Map(static r => (IReadOnlyList<Vector3d>)[r,])),
        ...
        UnifiedOperation.Apply(
            ...
            operation: (Func<(Vector3d[], Point3d[], Point3d[]), Result<IReadOnlyList<Curve[]>>>)(data =>
                FieldsCompute.IntegrateStreamlines(
                    vectorField: data.Item1,
                    gridPoints: data.Item2,
                    seeds: data.Item3,
                    stepSize: sampling.StepSize,
                    scheme: scheme,
                    resolution: sampling.Resolution,
                    bounds: bounds,
                    context: context).Map(static r => (IReadOnlyList<Curve[]>)[r,])),
        ...
        UnifiedOperation.Apply(
            ...
            operation: (Func<(double[], Point3d[], double[]), Result<IReadOnlyList<Mesh[]>>>)(data =>
                FieldsCompute.ExtractIsosurfaces(
                    scalarField: data.Item1,
                    gridPoints: data.Item2,
                    resolution: sampling.Resolution,
                    isovalues: data.Item3).Map(static r => (IReadOnlyList<Mesh[]>)[r,])),
        ...
        UnifiedOperation.Apply(
            ...
            operation: (Func<(double[], Vector3d[], double[,][], Point3d[]), Result<IReadOnlyList<Fields.CriticalPoint[]>>>)(data =>
                FieldsCompute.DetectCriticalPoints(
                    scalarField: data.Item1,
                    gradientField: data.Item2,
                    hessian: data.Item3,
                    grid: data.Item4,
                    resolution: sampling.Resolution).Map(static r => (IReadOnlyList<Fields.CriticalPoint[]>)[r,])),
        ...
        UnifiedOperation.Apply(
            ...
            operation: (Func<(double[], Point3d[]), Result<IReadOnlyList<Fields.FieldStatistics>>>)(data =>
                FieldsCompute.ComputeFieldStatistics(
                    scalarField: data.Item1,
                    grid: data.Item2).Map(static r => (IReadOnlyList<Fields.FieldStatistics>)[r,])),
    ...
    internal static Result<(Point3d[] Grid, double[] DirectionalDerivatives)> DirectionalDerivativeField(
        ...
        Vector3d direction,
        IGeometryContext context) {
        FieldsConfig.FieldOperationMetadata metadata = FieldsConfig.CompositionOperations[typeof(FieldsConfig.DirectionalDerivativeOp)];
        return UnifiedOperation.Apply(
            input: (gradientField, gridPoints),
            operation: (Func<(Vector3d[], Point3d[]), Result<IReadOnlyList<(Point3d[], double[])>>>)(field =>
                FieldsCompute.ComputeDirectionalDerivative(
                    gradientField: field.Item1,
                    grid: field.Item2,
                    direction: direction).Map(static r => (IReadOnlyList<(Point3d[], double[])>)[r,])),
            config: new OperationConfig<(Vector3d[], Point3d[]), (Point3d[], double[])> {
                Context = context,
                ValidationMode = metadata.ValidationMode,
                OperationName = metadata.OperationName,
    ...
    internal static Result<(Point3d[] Grid, double[] Magnitudes)> FieldMagnitude(
        ...
        Point3d[] gridPoints,
        IGeometryContext context) {
        FieldsConfig.FieldOperationMetadata metadata = FieldsConfig.CompositionOperations[typeof(FieldsConfig.MagnitudeOp)];
        return UnifiedOperation.Apply(
            input: (vectorField, gridPoints),
            operation: (Func<(Vector3d[], Point3d[]), Result<IReadOnlyList<(Point3d[], double[])>>>)(field =>
                FieldsCompute.ComputeFieldMagnitude(
                    vectorField: field.Item1,
                    grid: field.Item2).Map(static r => (IReadOnlyList<(Point3d[], double[])>)[r,])),
            config: new OperationConfig<(Vector3d[], Point3d[]), (Point3d[], double[])> {
                Context = context,
                ValidationMode = metadata.ValidationMode,
                OperationName = metadata.OperationName,
    ...
    internal static Result<(Point3d[] Grid, Vector3d[] Normalized)> NormalizeField(
        ...
        Point3d[] gridPoints,
        IGeometryContext context) {
        FieldsConfig.FieldOperationMetadata metadata = FieldsConfig.CompositionOperations[typeof(FieldsConfig.NormalizeOp)];
        return UnifiedOperation.Apply(
            input: (vectorField, gridPoints),
            operation: (Func<(Vector3d[], Point3d[]), Result<IReadOnlyList<(Point3d[], Vector3d[])>>>)(field =>
                FieldsCompute.NormalizeVectorField(
                    vectorField: field.Item1,
                    grid: field.Item2).Map(static r => (IReadOnlyList<(Point3d[], Vector3d[])>)[r,])),
            config: new OperationConfig<(Vector3d[], Point3d[]), (Point3d[], Vector3d[])> {
                Context = context,
                ValidationMode = metadata.ValidationMode,
                OperationName = metadata.OperationName,
    ...
    internal static Result<(Point3d[] Grid, double[] Product)> ScalarVectorProduct(
        ...
        IGeometryContext context) {
        FieldsConfig.FieldOperationMetadata metadata = FieldsConfig.CompositionOperations[typeof(FieldsConfig.ScalarVectorProductOp)];
        return UnifiedOperation.Apply(
            ...
            operation: (Func<(double[], Vector3d[], Point3d[]), Result<IReadOnlyList<(Point3d[], double[])>>>)(field =>
                FieldsCompute.ScalarVectorProduct(
                    scalarField: field.Item1,
                    vectorField: field.Item2,
                    grid: field.Item3,
                    component: component).Map(static r => (IReadOnlyList<(Point3d[], double[])>)[r,])),
    ...
    internal static Result<(Point3d[] Grid, double[] DotProduct)> VectorDotProduct(
        ...
        Point3d[] gridPoints,
        IGeometryContext context) {
        FieldsConfig.FieldOperationMetadata metadata = FieldsConfig.CompositionOperations[typeof(FieldsConfig.VectorDotProductOp)];
        return UnifiedOperation.Apply(
            ...
            operation: (Func<(Vector3d[], Vector3d[], Point3d[]), Result<IReadOnlyList<(Point3d[], double[])>>>)(field =>
                FieldsCompute.VectorDotProduct(
                    vectorField1: field.Item1,
                    vectorField2: field.Item2,
                    grid: field.Item3).Map(static r => (IReadOnlyList<(Point3d[], double[])>)[r,])),
