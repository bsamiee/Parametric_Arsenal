...
internal static class FieldsCore {
    /// <summary>Unified operation dispatch table: (operation type, geometry type) → (execute function, validation mode, buffer size).</summary>
    internal static readonly FrozenDictionary<(Type OperationType, Type GeometryType), (Func<object, Fields.FieldSpec, IGeometryContext, Result<(Point3d[], double[])>> Execute, V ValidationMode, int BufferSize)> OperationRegistry =
        new Dictionary<(Type, Type), (Func<object, Fields.FieldSpec, IGeometryContext, Result<(Point3d[], double[])>>, V, int)> {
            [(typeof(Fields.DistanceFieldOperation), typeof(Mesh))] = (ExecuteDistanceField<Mesh>, V.Standard | V.MeshSpecific, 4096),
            [(typeof(Fields.DistanceFieldOperation), typeof(Brep))] = (ExecuteDistanceField<Brep>, V.Standard | V.Topology, 8192),
            [(typeof(Fields.DistanceFieldOperation), typeof(Curve))] = (ExecuteDistanceField<Curve>, V.Standard | V.Degeneracy, 2048),
            [(typeof(Fields.DistanceFieldOperation), typeof(Surface))] = (ExecuteDistanceField<Surface>, V.Standard | V.BoundingBox, 4096),
        }.ToFrozenDictionary();
    ...
    private static Result<(Point3d[], double[])> ExecuteDistanceField<T>(
        ...
        ((Func<Result<(Point3d[], double[])>>)(() => {
            ...
            int bufferSize = OperationRegistry.TryGetValue((typeof(Fields.DistanceFieldOperation), typeof(T)), out (Func<object, Fields.FieldSpec, IGeometryContext, Result<(Point3d[], double[])>> Execute, V ValidationMode, int BufferSize) config)
