# Fields Library Blueprint

## Overview
Scalar and vector field operations for computational field analysis: signed distance fields (SDF), gradient fields, streamline tracing via RK4 integration, and isosurface extraction via marching cubes. Enables distance-based shape analysis, flow field visualization, and implicit surface modeling for generative design and computational analysis.
...
**Types** (2 total):
- `Fields` (static class): Primary API entry point dispatching to FieldsCore via byte operation codes
...
public static Result<(Point3d[] Grid, double[] Distances)> DistanceField<T>(
    ...
    FieldsCore.OperationRegistry.TryGetValue(typeof(T), out (V mode, Func<object, FieldSpec, IGeometryContext, Result<(Point3d[], double[])>> execute) config) switch {
...
### File 2: `FieldsCore.cs`
**Purpose**: FrozenDictionary dispatch and spatial acceleration

**Types** (3 total):
- `FieldsCore` (internal static class): Dispatch registry and RTree factories
...
### File 3: `FieldsCompute.cs`
**Purpose**: Core algorithms - distance, gradient, streamline, isosurface

**Types** (3 total):
- `FieldsCompute` (internal static class): Computational kernels
...
### File 4: `FieldsConfig.cs`
**Purpose**: Configuration constants, byte-based operation IDs, and FrozenDictionary dispatch tables

**Types** (1 total):
- `FieldsConfig` (internal static class): Constants, byte operation codes, marching cubes lookup, buffer size dispatch
...
```csharp
// FieldsCore.cs dispatch registry
internal static readonly FrozenDictionary<(byte Operation, Type Geometry), (V Mode, Func<object, FieldSpec, IGeometryContext, Result<T>> Execute)> OperationRegistry =
    new Dictionary<(byte, Type), (V, Func<object, FieldSpec, IGeometryContext, Result<T>>)> {
        [(FieldsConfig.OperationDistance, typeof(Mesh))] = (V.Standard | V.MeshSpecific, ExecuteDistanceField<Mesh>),
        [(FieldsConfig.OperationDistance, typeof(Brep))] = (V.Standard | V.Topology, ExecuteDistanceField<Brep>),
        [(FieldsConfig.OperationDistance, typeof(Curve))] = (V.Standard | V.Degeneracy, ExecuteDistanceField<Curve>),
        [(FieldsConfig.OperationDistance, typeof(Surface))] = (V.Standard | V.BoundingBox, ExecuteDistanceField<Surface>),
        [(FieldsConfig.OperationGradient, typeof(Mesh))] = (V.Standard | V.MeshSpecific, ExecuteGradientField<Mesh>),
...
byte operationType = FieldsConfig.OperationDistance;  // NOT enum, NOT string
FieldsCore.OperationRegistry.TryGetValue((operationType, typeof(T)), out (V mode, Func<...> execute) config) switch {
    true => UnifiedOperation.Apply(...),
...
**Range 2700-2749** (Fields operations within Geometry domain 2000-2999):
...
3. **Create FieldsConfig.cs FIRST**: Byte operation codes, FrozenDictionary dispatch, marching cubes table
4. **Add error codes**: Add fields errors to E.cs (2700-2749 range)
5. **Create FieldsCore.cs**: Byte-based FrozenDictionary dispatch registry, RTree factories
6. **Create FieldsCompute.cs**: Core algorithms with ArrayPool buffers
...
11. **Create Fields.cs**: Public API dispatching via byte operation codes
12. **VERIFY NO ENUMS**: `grep -r "enum " fields/ --include="*.cs"` must return zero results
