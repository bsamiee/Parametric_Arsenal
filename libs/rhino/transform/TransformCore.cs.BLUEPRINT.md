# TransformCore.cs Blueprint

## File Purpose
Core implementation: transform matrix building, validation, geometry application, and array generation algorithms.
...
internal static class TransformCore {
    ...
    internal static Result<Rhino.Geometry.Transform> BuildTransform(
        ...
        spec switch {
            { Matrix: Rhino.Geometry.Transform m } =>
            ...
            { UniformScale: (Point3d anchor, double factor) } =>
                factor >= TransformConfig.MinScaleFactor && factor <= TransformConfig.MaxScaleFactor
                    ...
                    : ResultFactory.Create<Rhino.Geometry.Transform>(error: E.Transform.InvalidScaleFactor.WithContext($"Factor: {factor:F6}")),
            ...
            { NonUniformScale: (Plane plane, double x, double y, double z) } =>
                ...
                && z >= TransformConfig.MinScaleFactor && z <= TransformConfig.MaxScaleFactor
                    ...
                    : ResultFactory.Create<Rhino.Geometry.Transform>(error: E.Transform.InvalidScaleFactor.WithContext($"Plane valid: {plane.IsValid}, X: {x:F6}, Y: {y:F6}, Z: {z:F6}")),
            ...
            { Rotation: (double angle, Vector3d axis, Point3d center) } =>
                axis.Length > context.AbsoluteTolerance
                    ...
                    : ResultFactory.Create<Rhino.Geometry.Transform>(error: E.Transform.InvalidRotationAxis.WithContext($"Axis length: {axis.Length:F6}")),
            ...
            { RotationVectors: (Vector3d start, Vector3d end, Point3d center) } =>
                start.Length > context.AbsoluteTolerance && end.Length > context.AbsoluteTolerance
                    ? ResultFactory.Create(value: Rhino.Geometry.Transform.Rotation(start, end, center))
                    : ResultFactory.Create<Rhino.Geometry.Transform>(error: E.Transform.InvalidRotationAxis.WithContext($"Start: {start.Length:F6}, End: {end.Length:F6}")),
            ...
            { Shear: (Plane plane, Vector3d direction, double angle) } =>
                ...
                && !plane.ZAxis.IsParallelTo(direction, context.AngleTolerance * TransformConfig.AngleToleranceMultiplier)
                    ...
                    : ResultFactory.Create<Rhino.Geometry.Transform>(error: E.Transform.InvalidShearParameters.WithContext($"Plane: {plane.IsValid}, Direction: {direction.Length:F6}")),

            { ProjectionPlane: Plane plane } =>
                plane.IsValid
                    ? ResultFactory.Create(value: Rhino.Geometry.Transform.PlanarProjection(plane))
                    : ResultFactory.Create<Rhino.Geometry.Transform>(error: E.Transform.InvalidProjectionPlane),

            { ChangeBasis: (Plane from, Plane to) } =>
                from.IsValid && to.IsValid
                    ? ResultFactory.Create(value: Rhino.Geometry.Transform.ChangeBasis(from, to))
                    : ResultFactory.Create<Rhino.Geometry.Transform>(error: E.Transform.InvalidBasisPlanes.WithContext($"From: {from.IsValid}, To: {to.IsValid}")),

            { PlaneToPlane: (Plane from, Plane to) } =>
                from.IsValid && to.IsValid
                    ? ResultFactory.Create(value: Rhino.Geometry.Transform.PlaneToPlane(from, to))
                    : ResultFactory.Create<Rhino.Geometry.Transform>(error: E.Transform.InvalidBasisPlanes.WithContext($"From: {from.IsValid}, To: {to.IsValid}")),
    ...
    private static Result<Rhino.Geometry.Transform> ValidateTransform(
        ...
        transform.IsValid && Math.Abs(transform.Determinant) > context.AbsoluteTolerance
            ...
            : ResultFactory.Create<Rhino.Geometry.Transform>(
    ...
    internal static Result<IReadOnlyList<T>> RectangularArray<T>(
        ...
        && (zCount is 1 || Math.Abs(zSpacing) > context.AbsoluteTolerance)
            ? ((Func<Result<IReadOnlyList<T>>>)(() => {
                ...
                Rhino.Geometry.Transform[] transforms = new Rhino.Geometry.Transform[totalCount];
                int index = 0;
                ...
                for (int i = 0; i < xCount; i++) {
                    ...
                    for (int j = 0; j < yCount; j++) {
                        ...
                        for (int k = 0; k < zCount; k++) {
                            ...
                            transforms[index++] = Rhino.Geometry.Transform.Translation(dx: dx, dy: dy, dz: dz);
                        }
                    }
                }
        ...
        && Math.Abs(spacing) > context.AbsoluteTolerance
            ? ((Func<Result<IReadOnlyList<T>>>)(() => {
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    transforms[i] = Rhino.Geometry.Transform.Translation(motion);
                }
...
## Notes
- BuildTransform uses exhaustive switch expression on TransformSpec
- Each transform type validates parameters before RhinoCommon SDK call
- ValidateTransform checks IsValid and non-zero determinant
- ApplyTransform handles Extrusion → Brep conversion with disposal
- Rectangular/Polar/Linear array use pre-computed transform arrays
- For loops optimized for hot paths (array generation)
- Inline lambda execution pattern: `((Func<Result<T>>)(() => { ... }))()`
- UnifiedOperation for batch transform application with error handling
- All validation uses RhinoMath constants and context tolerances
