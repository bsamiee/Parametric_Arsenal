# Transform.cs Blueprint

## File Purpose
Public API surface with nested types for unified transform operations via polymorphic dispatch.
...
## Primary API Class & Nested Types
...
### Nested Type 1: TransformSpec (Discriminated Union)
```csharp
    ...
    public readonly record struct TransformSpec {
        ...
        [Pure]
        public static TransformSpec Rotate(Vector3d start, Vector3d end, Point3d center) => new() { RotationVectors = (start, end, center) };
        ...
        [Pure]
        public static TransformSpec Shear(Plane plane, Vector3d direction, double angle) => new() { Shear = (plane, direction, angle) };
        ...
        [Pure]
        public static TransformSpec ChangeBasis(Plane from, Plane to) => new() { ChangeBasis = (from, to) };
        /// <summary>Create plane-to-plane specification.</summary>
        [Pure]
        public static TransformSpec PlaneToPlane(Plane from, Plane to) => new() { PlaneToPlane = (from, to) };
    }
```

### Nested Type 2: ArraySpec
```csharp
...
### Apply Method (Unified Entry Point)
```csharp
...
## Type-Specific Ergonomic Overloads
...
```csharp
    ...
    public static Result<T> Scale<T>(
        ...
        double zScale,
        IGeometryContext context) where T : GeometryBase =>
        Apply(geometry: geometry, spec: TransformSpec.Scale(plane, xScale, yScale, zScale), context: context);
    ...
    public static Result<T> Rotate<T>(
        ...
        Point3d center,
        IGeometryContext context) where T : GeometryBase =>
        Apply(geometry: geometry, spec: TransformSpec.Rotate(angleRadians, axis, center), context: context);
    ...
    public static Result<T> Rotate<T>(
        ...
        Point3d center,
        IGeometryContext context) where T : GeometryBase =>
        Apply(geometry: geometry, spec: TransformSpec.Rotate(startDirection, endDirection, center), context: context);
    ...
    public static Result<T> Mirror<T>(
        ...
        Plane plane,
        IGeometryContext context) where T : GeometryBase =>
        Apply(geometry: geometry, spec: TransformSpec.Mirror(plane), context: context);
    ...
    public static Result<T> Translate<T>(
        ...
        Vector3d motion,
        IGeometryContext context) where T : GeometryBase =>
        Apply(geometry: geometry, spec: TransformSpec.Translate(motion), context: context);
    ...
    public static Result<T> Translate<T>(
        ...
        Point3d end,
        IGeometryContext context) where T : GeometryBase =>
        Apply(geometry: geometry, spec: TransformSpec.Translate(end - start), context: context);
    ...
    public static Result<T> Shear<T>(
        ...
        double angle,
        IGeometryContext context) where T : GeometryBase =>
        Apply(geometry: geometry, spec: TransformSpec.Shear(plane, direction, angle), context: context);
    ...
    public static Result<T> Project<T>(
        ...
        Plane plane,
        IGeometryContext context) where T : GeometryBase =>
        Apply(geometry: geometry, spec: TransformSpec.Project(plane), context: context);
    ...
    public static Result<T> ChangeBasis<T>(
        ...
        Plane toPlane,
        IGeometryContext context) where T : GeometryBase =>
        Apply(geometry: geometry, spec: TransformSpec.ChangeBasis(fromPlane, toPlane), context: context);
    ...
    public static Result<T> PlaneToPlane<T>(
        ...
        Plane toPlane,
        IGeometryContext context) where T : GeometryBase =>
        Apply(geometry: geometry, spec: TransformSpec.PlaneToPlane(fromPlane, toPlane), context: context);
}
```

## File Closure
```csharp
// End of Transform class and namespace
```

## Notes
- All nested types within Transform class to avoid type sprawl
- TransformSpec uses discriminated union pattern with nullable fields
- ArraySpec and MorphSpec use byte-based mode dispatch
- All factory methods are Pure and marked appropriately
- Type-specific overloads delegate to primary Apply method
- StructLayout(LayoutKind.Auto) for optimal memory layout
- Namespace suppression attribute on main class only
