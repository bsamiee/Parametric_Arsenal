            ...
            Brep b => b.Faces.Count is 0
                ...
                : ResultFactory.Create(value: (
                    Primitives: b.Faces
                            ...
                            .Select<Surface, (bool Success, (byte Type, Plane Frame, double[] Params) Primitive)>(s => (
                                ...
                                Primitive: s.TryGetPlane(out Plane pl, tolerance: ExtractionConfig.PrimitiveFitTolerance) ? (Type: 0, Frame: pl, Params: [pl.OriginX, pl.OriginY, pl.OriginZ,]) : s.TryGetCylinder(out Cylinder cy, tolerance: ExtractionConfig.PrimitiveFitTolerance) ? (Type: 1, Frame: new Plane(cy.Center, cy.Axis), Params: [cy.Radius, cy.TotalHeight,]) : s.TryGetSphere(out Sphere sph, tolerance: ExtractionConfig.PrimitiveFitTolerance) ? (Type: 2, Frame: new Plane(sph.Center, Vector3d.ZAxis), Params: [sph.Radius,]) : (Type: 3, Frame: Plane.WorldXY, Params: [])
                            ))
                            .Where(t => t.Success)
        ...
        geometries.Length < ExtractionConfig.PatternMinInstances
            ...
            : geometries.Select(g => g.GetBoundingBox(accurate: false).Center).ToArray() is Point3d[] pts && pts.Length >= 2
                ? pts.Zip(pts.Skip(1), (a, b) => b - a).ToArray() is Vector3d[] deltas && deltas.All(d => (d - deltas[0]).Length < 0.001)
                    ...
                    : pts.Skip(1).Select(p => Vector3d.VectorAngle(pts[0] - pts[1], p - pts[1])).ToArray() is double[] angles && angles.Length > 0 && angles.All(a => Math.Abs(a - angles[0]) < ExtractionConfig.SymmetryAngleTolerance)
