...
internal static class ExtractionCompute {
    ...
    [Pure]
    internal static Result<(byte Type, RhinoTransform SymmetryTransform, double Confidence)> ExtractPatterns(GeometryBase[] geometries, IGeometryContext context) =>
    ...
    private static Result<(byte Type, RhinoTransform SymmetryTransform, double Confidence)> DetectPatternType(Point3d[] centers, IGeometryContext context) =>
        ((Func<Vector3d[]>)(() => {
            ...
            ? ResultFactory.Create(value: (Type: ExtractionConfig.PatternTypeLinear, SymmetryTransform: RhinoTransform.Translation(deltas[0]), Confidence: 1.0))
            : TryDetectRadialPattern(centers: centers, context: context) is Result<(byte, RhinoTransform, double)> radialResult && radialResult.IsSuccess
                ? radialResult
                : TryDetectGridPattern(centers: centers, context: context) is Result<(byte, RhinoTransform, double)> gridResult && gridResult.IsSuccess
                    ? gridResult
                    : TryDetectScalingPattern(centers: centers, context: context) is Result<(byte, RhinoTransform, double)> scaleResult && scaleResult.IsSuccess
                        ? scaleResult
                        : ResultFactory.Create<(byte, RhinoTransform, double)>(error: E.Geometry.NoPatternDetected.WithContext("No linear, radial, grid, or scaling pattern detected"));

    private static Result<(byte Type, RhinoTransform SymmetryTransform, double Confidence)> TryDetectRadialPattern(Point3d[] centers, IGeometryContext context) {
        ...
        return meanDistance > context.AbsoluteTolerance
            ...
            && angles.All(a => RhinoMath.EpsilonEquals(a, meanAngle, ExtractionConfig.RadialAngleVariationThreshold))
                ? ResultFactory.Create(value: (Type: ExtractionConfig.PatternTypeRadial, SymmetryTransform: RhinoTransform.Rotation(meanAngle, normal, centroid), Confidence: 0.9))
                : ResultFactory.Create<(byte, RhinoTransform, double)>(error: E.Geometry.NoPatternDetected);
    }
    ...
    private static Result<(byte Type, RhinoTransform SymmetryTransform, double Confidence)> TryDetectGridPattern(Point3d[] centers, IGeometryContext context) =>
        (centers[0], Enumerable.Range(0, centers.Length - 1).Select(i => centers[i + 1] - centers[0]).ToArray()) is (Point3d origin, Vector3d[] relativeVectors)
            ...
            && relativeVectors.All(vec => IsGridPoint(vector: vec, u: u, v: v, context: context))
                ? ResultFactory.Create(value: (Type: ExtractionConfig.PatternTypeGrid, SymmetryTransform: RhinoTransform.PlaneToPlane(Plane.WorldXY, new Plane(origin, u, v)), Confidence: 0.9))
                : ResultFactory.Create<(byte, RhinoTransform, double)>(error: E.Geometry.NoPatternDetected);
    ...
    private static Result<(byte Type, RhinoTransform SymmetryTransform, double Confidence)> TryDetectScalingPattern(Point3d[] centers, IGeometryContext context) =>
        new Point3d(centers.Average(p => p.X), centers.Average(p => p.Y), centers.Average(p => p.Z)) is Point3d centroid
            ...
            && validRatios.Length >= 2 && ComputeVariance(values: validRatios) is double variance && variance < ExtractionConfig.ScalingVarianceThreshold
                ? ResultFactory.Create(value: (Type: ExtractionConfig.PatternTypeScaling, SymmetryTransform: RhinoTransform.Scale(anchor: centroid, scaleFactor: validRatios.Average()), Confidence: 0.7))
                : ResultFactory.Create<(byte, RhinoTransform, double)>(error: E.Geometry.NoPatternDetected);
