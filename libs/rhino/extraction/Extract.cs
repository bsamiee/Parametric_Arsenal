...
public static class Extract {
    ...
    public static Result<IReadOnlyList<Point3d>> Points<T>(T input, object spec, IGeometryContext context) where T : GeometryBase =>
        spec switch {
            int c when c <= 0 => ResultFactory.Create<IReadOnlyList<Point3d>>(input: ExtractionErrors.Operation.InvalidCount),
            double l when l <= 0 => ResultFactory.Create<IReadOnlyList<Point3d>>(input: ExtractionErrors.Operation.InvalidLength),
            (int c, bool) when c <= 0 => ResultFactory.Create<IReadOnlyList<Point3d>>(input: ExtractionErrors.Operation.InvalidCount),
            (double l, bool) when l <= 0 => ResultFactory.Create<IReadOnlyList<Point3d>>(input: ExtractionErrors.Operation.InvalidLength),
            Vector3d dir when dir.Length <= context.AbsoluteTolerance => ResultFactory.Create<IReadOnlyList<Point3d>>(input: ExtractionErrors.Operation.InvalidDirection),
            ...
            _ => ResultFactory.Create<IReadOnlyList<Point3d>>(input: ExtractionErrors.Operation.InvalidMethod),
        };
}
