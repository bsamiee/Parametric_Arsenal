...
public static class Extract {
    ...
    public static Result<IReadOnlyList<Point3d>> Points<T>(T input, object spec, IGeometryContext context) where T : GeometryBase =>
        spec switch {
            int c when c <= 0 => ResultFactory.Create<IReadOnlyList<Point3d>>(error: ErrorRegistry.Get(4201)),
            double l when l <= 0 => ResultFactory.Create<IReadOnlyList<Point3d>>(error: ErrorRegistry.Get(4202)),
            (int c, bool) when c <= 0 => ResultFactory.Create<IReadOnlyList<Point3d>>(error: ErrorRegistry.Get(4201)),
            (double l, bool) when l <= 0 => ResultFactory.Create<IReadOnlyList<Point3d>>(error: ErrorRegistry.Get(4202)),
            Vector3d dir when dir.Length <= context.AbsoluteTolerance => ResultFactory.Create<IReadOnlyList<Point3d>>(error: ErrorRegistry.Get(4203)),
            ...
            _ => ResultFactory.Create<IReadOnlyList<Point3d>>(error: ErrorRegistry.Get(4204)),
        };
}
