...
public static class Extract {
    ...
    public static Result<IReadOnlyList<Point3d>> Points<T>(T input, object spec, IGeometryContext context) where T : GeometryBase =>
        spec switch {
            int c when c <= 0 => ResultFactory.Create<IReadOnlyList<Point3d>>(error: ErrorFactory.Create(code: 2002)),
            double l when l <= 0 => ResultFactory.Create<IReadOnlyList<Point3d>>(error: ErrorFactory.Create(code: 2003)),
            (int c, bool) when c <= 0 => ResultFactory.Create<IReadOnlyList<Point3d>>(error: ErrorFactory.Create(code: 2002)),
            (double l, bool) when l <= 0 => ResultFactory.Create<IReadOnlyList<Point3d>>(error: ErrorFactory.Create(code: 2003)),
            Vector3d dir when dir.Length <= context.AbsoluteTolerance => ResultFactory.Create<IReadOnlyList<Point3d>>(error: ErrorFactory.Create(code: 2004)),
            int or double or (int, bool) or (double, bool) or Vector3d or Continuity or Semantic =>
                UnifiedOperation.Apply(
                    ...
                    new OperationConfig<T, Point3d> { Context = context, ValidationMode = Modes.None }),
            _ => ResultFactory.Create<IReadOnlyList<Point3d>>(error: ErrorFactory.Create(code: 2000)),
        };
}
