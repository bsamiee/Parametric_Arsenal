...
internal static class ExtractionStrategies {
    ...
    public static Result<IReadOnlyList<Point3d>> Extract(
        ...
        ResultFactory.Create(value: geometry)
            .Validate(args: [context, GetValidationMode(geometry.GetType(), method)])
    ...
    /// <summary>Walks type hierarchy to find validation mode with full inheritance chain traversal.</summary>
    [Pure, MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static ValidationMode GetValidationMode(Type geometryType, ExtractionMethod method) {
        Type? currentType = geometryType;
        while (currentType is not null && currentType != typeof(object)) {
            if (_validationConfig.TryGetValue((method, currentType), out ValidationMode mode)) {
                return mode;
            }
    ...
    private static Point3d[]? ExtractCore(
        ...
        (method, geometry) switch {
            (ExtractionMethod.Analytical or ExtractionMethod.EdgeMidpoints, Extrusion extrusion) =>
                ExtractWithBrepConversion(extrusion.ToBrep(splitKinkyFaces: true), method, context, count, length, includeEnds),
            (ExtractionMethod.Analytical or ExtractionMethod.EdgeMidpoints, SubD subD) =>
                ExtractWithBrepConversion(subD.ToBrep(), method, context, count, length, includeEnds),
            _ => ExtractFromNormalized(geometry, method, context, count, length, includeEnds),
        };
...
#pragma warning disable IDISP007 // Don't dispose injected - we own the Brep created by ToBrep()
    private static Point3d[]? ExtractWithBrepConversion(
        ...
        try {
            return ExtractFromNormalized(brep, method, context, count, length, includeEnds);
        } finally {
            brep.Dispose();
        }
    }
#pragma warning restore IDISP007
    ...
    /// <summary>Extracts points from normalized geometry using method-specific dispatch.</summary>
    [Pure]
    private static Point3d[]? ExtractFromNormalized(
        GeometryBase geometry, ExtractionMethod method,
        IGeometryContext context, int? count, double? length, bool includeEnds) =>
        (method, geometry) switch {
            ...
            (ExtractionMethod.Analytical, GeometryBase analytic) => ExtractAnalytical(analytic),
    ...
    private static Point3d[]? ExtractQuadrant(Curve curve, IGeometryContext context) =>
        curve switch {
            ...
            _ when curve.TryGetEllipse(out Ellipse ellipse, context.AbsoluteTolerance) =>
                [ellipse.Center + (ellipse.Plane.XAxis * ellipse.Radius1),
                    ellipse.Center + (ellipse.Plane.YAxis * ellipse.Radius2),
                    ellipse.Center - (ellipse.Plane.XAxis * ellipse.Radius1),
                    ellipse.Center - (ellipse.Plane.YAxis * ellipse.Radius2),],
            _ when curve.TryGetPolyline(out Polyline polyline) => [.. polyline,],
            _ when curve.IsLinear(context.AbsoluteTolerance) => [curve.PointAtStart, curve.PointAtEnd,],
            _ => null,
        };
}
