...
internal static class ExtractionCore {
    ...
    private static readonly (FrozenDictionary<(ExtractionConfig.OpType Operation, Type GeometryType), Func<GeometryBase, Extract.Request, IGeometryContext, Result<Point3d[]>>> Map,
        FrozenDictionary<ExtractionConfig.OpType, (Type GeometryType, Func<GeometryBase, Extract.Request, IGeometryContext, Result<Point3d[]>> Handler)[]> Fallbacks) _pointRegistry = BuildHandlerRegistry();

    private static readonly FrozenDictionary<(ExtractionConfig.OpType Operation, Type GeometryType), Func<GeometryBase, Extract.Request, IGeometryContext, Result<Point3d[]>>> _handlers = _pointRegistry.Map;
    private static readonly FrozenDictionary<ExtractionConfig.OpType, (Type GeometryType, Func<GeometryBase, Extract.Request, IGeometryContext, Result<Point3d[]>> Handler)[]> _handlerFallbacks = _pointRegistry.Fallbacks;

    /// <summary>(Kind, Type) to curve handler function mapping for O(1) dispatch.</summary>
    private static readonly (FrozenDictionary<(ExtractionConfig.OpType Operation, Type GeometryType), Func<GeometryBase, Extract.Request, IGeometryContext, Result<Curve[]>>> Map,
        FrozenDictionary<ExtractionConfig.OpType, (Type GeometryType, Func<GeometryBase, Extract.Request, IGeometryContext, Result<Curve[]>> Handler)[]> Fallbacks) _curveRegistry = BuildCurveHandlerRegistry();

    private static readonly FrozenDictionary<(ExtractionConfig.OpType Operation, Type GeometryType), Func<GeometryBase, Extract.Request, IGeometryContext, Result<Curve[]>>> _curveHandlers = _curveRegistry.Map;
    private static readonly FrozenDictionary<ExtractionConfig.OpType, (Type GeometryType, Func<GeometryBase, Extract.Request, IGeometryContext, Result<Curve[]>> Handler)[]> _curveHandlerFallbacks = _curveRegistry.Fallbacks;
    ...
    internal static Result<IReadOnlyList<Curve>> ExecuteCurves(GeometryBase geometry, Extract.Request request, IGeometryContext context) =>
        _curveHandlers.TryGetValue((request.Operation, geometry.GetType()), out Func<GeometryBase, Extract.Request, IGeometryContext, Result<Curve[]>>? handler)
            ? handler(geometry, request, context).Map(curves => (IReadOnlyList<Curve>)curves)
            : _curveHandlerFallbacks.TryGetValue(request.Operation, out (Type GeometryType, Func<GeometryBase, Extract.Request, IGeometryContext, Result<Curve[]>> Handler)[]? fallbacks)
    ...
    private static Result<IReadOnlyList<Point3d>> ExecuteWithNormalization(
        ...
        (GeometryBase? candidate, bool shouldDispose) = (geometry, request.Operation) switch {
            (Extrusion extrusion, ExtractionConfig.OpType.Analytical or ExtractionConfig.OpType.EdgeMidpoints or ExtractionConfig.OpType.FaceCentroids or ExtractionConfig.OpType.DivideByCount or ExtractionConfig.OpType.DivideByLength or ExtractionConfig.OpType.DirectionalExtrema) => (extrusion.ToBrep(splitKinkyFaces: true), true),
            (SubD subd, ExtractionConfig.OpType.Analytical or ExtractionConfig.OpType.EdgeMidpoints or ExtractionConfig.OpType.FaceCentroids or ExtractionConfig.OpType.DivideByCount or ExtractionConfig.OpType.DivideByLength or ExtractionConfig.OpType.DirectionalExtrema) => (subd.ToBrep(), true),
        ...
        return candidate is null
            ...
            : ((Func<Result<IReadOnlyList<Point3d>>>)(() => {
                ...
                V mode = ExtractionConfig.GetOperationMeta(request.Operation, normalized.GetType()).ValidationMode;
    ...
    private static Result<Point3d[]> DispatchExtraction(GeometryBase geometry, Extract.Request request, IGeometryContext context) =>
        _handlers.TryGetValue((request.Operation, geometry.GetType()), out Func<GeometryBase, Extract.Request, IGeometryContext, Result<Point3d[]>>? handler)
            ? handler(geometry, request, context)
            : _handlerFallbacks.TryGetValue(request.Operation, out (Type GeometryType, Func<GeometryBase, Extract.Request, IGeometryContext, Result<Point3d[]>> Handler)[]? fallbacks)
                ? InvokeFallback(geometry, request, context, fallbacks)
    ...
    [Pure]
    private static (FrozenDictionary<(ExtractionConfig.OpType Operation, Type GeometryType), Func<GeometryBase, Extract.Request, IGeometryContext, Result<Point3d[]>>> Map,
        FrozenDictionary<ExtractionConfig.OpType, (Type GeometryType, Func<GeometryBase, Extract.Request, IGeometryContext, Result<Point3d[]>> Handler)[]> Fallbacks) BuildHandlerRegistry() {
        Dictionary<(ExtractionConfig.OpType Operation, Type GeometryType), Func<GeometryBase, Extract.Request, IGeometryContext, Result<Point3d[]>>> map = new() {
            [((ExtractionConfig.OpType)1, typeof(Brep))] = static (geometry, _, _) => geometry is Brep brep
                ? ((Func<Result<Point3d[]>>)(() => {
            ...
            [((ExtractionConfig.OpType)1, typeof(Curve))] = static (geometry, _, _) => geometry is Curve curve
                ? ((Func<Result<Point3d[]>>)(() => {
            ...
            [((ExtractionConfig.OpType)1, typeof(Surface))] = static (geometry, _, _) => geometry is Surface surface
                ? ((Func<Result<Point3d[]>>)(() => {
            ...
            [((ExtractionConfig.OpType)1, typeof(Mesh))] = static (geometry, _, _) => geometry is Mesh mesh
                ? ((Func<Result<Point3d[]>>)(() => {
            ...
            [((ExtractionConfig.OpType)1, typeof(PointCloud))] = static (geometry, _, _) => geometry is PointCloud cloud && cloud.GetPoints() is Point3d[] cloudPoints && cloudPoints.Length > 0
            ...
            [((ExtractionConfig.OpType)2, typeof(Curve))] = static (geometry, _, _) => geometry is Curve curve
                ? ResultFactory.Create<Point3d[]>(value: [curve.PointAtStart, curve.PointAtEnd,])
                : ResultFactory.Create<Point3d[]>(error: E.Geometry.InvalidExtraction.WithContext("Expected Curve")),
            [((ExtractionConfig.OpType)2, typeof(Surface))] = static (geometry, _, _) => geometry is Surface surface && (surface.Domain(0), surface.Domain(1)) is (Interval uDomain, Interval vDomain)
                ? ResultFactory.Create<Point3d[]>(value: [surface.PointAt(uDomain.Min, vDomain.Min), surface.PointAt(uDomain.Max, vDomain.Min), surface.PointAt(uDomain.Max, vDomain.Max), surface.PointAt(uDomain.Min, vDomain.Max),])
                : ResultFactory.Create<Point3d[]>(error: E.Geometry.InvalidExtraction.WithContext("Surface domain unavailable")),
            [((ExtractionConfig.OpType)2, typeof(GeometryBase))] = static (geometry, _, _) => ResultFactory.Create(value: geometry.GetBoundingBox(accurate: true).GetCorners()),
            [((ExtractionConfig.OpType)3, typeof(NurbsCurve))] = static (geometry, _, _) => geometry is NurbsCurve nurbsCurve && nurbsCurve.GrevillePoints() is Point3dList greville
            ...
            [((ExtractionConfig.OpType)3, typeof(NurbsSurface))] = static (geometry, _, _) => geometry is NurbsSurface nurbsSurface && nurbsSurface.Points is NurbsSurfacePointList controlPoints
            ...
            [((ExtractionConfig.OpType)3, typeof(Curve))] = static (geometry, _, _) => geometry is Curve curve && curve.ToNurbsCurve() is NurbsCurve nurbs
                ? ((Func<Result<Point3d[]>>)(() => {
                    try {
            ...
            [((ExtractionConfig.OpType)3, typeof(Surface))] = static (geometry, _, _) => geometry is Surface surface && surface.ToNurbsSurface() is NurbsSurface nurbs && nurbs.Points is not null
            ...
            [((ExtractionConfig.OpType)4, typeof(NurbsCurve))] = static (geometry, _, _) => geometry is NurbsCurve nurbs && nurbs.InflectionPoints() is Point3d[] inflections
            ...
            [((ExtractionConfig.OpType)4, typeof(Curve))] = static (geometry, _, _) => geometry is Curve curve && curve.ToNurbsCurve() is NurbsCurve nurbs
            ...
            [((ExtractionConfig.OpType)5, typeof(Curve))] = static (geometry, _, context) => geometry is Curve curve && context.AbsoluteTolerance is double tolerance
            ...
            [((ExtractionConfig.OpType)6, typeof(Brep))] = static (geometry, _, _) => geometry is Brep brep
                ? ResultFactory.Create<Point3d[]>(value: [.. brep.Edges.Select(static edge => edge.PointAtNormalizedLength(0.5)),])
                : ResultFactory.Create<Point3d[]>(error: E.Geometry.InvalidExtraction.WithContext("Expected Brep")),
            [((ExtractionConfig.OpType)6, typeof(Mesh))] = static (geometry, _, _) => geometry is Mesh mesh
                ...
                : ResultFactory.Create<Point3d[]>(error: E.Geometry.InvalidExtraction.WithContext("Expected Mesh")),
            [((ExtractionConfig.OpType)6, typeof(Curve))] = static (geometry, _, _) => geometry is Curve curve
                ...
                : ResultFactory.Create<Point3d[]>(error: E.Geometry.InvalidExtraction.WithContext("Expected Curve")),
            [((ExtractionConfig.OpType)7, typeof(Brep))] = static (geometry, _, _) => geometry is Brep brep
                ...
                : ResultFactory.Create<Point3d[]>(error: E.Geometry.InvalidExtraction.WithContext("Expected Brep")),
            [((ExtractionConfig.OpType)7, typeof(Mesh))] = static (geometry, _, _) => geometry is Mesh mesh
                ...
                : ResultFactory.Create<Point3d[]>(error: E.Geometry.InvalidExtraction.WithContext("Expected Mesh")),
            [((ExtractionConfig.OpType)10, typeof(Curve))] = static (geometry, request, _) => geometry is Curve curve && request.Parameter is int count
            ...
            [((ExtractionConfig.OpType)10, typeof(Surface))] = static (geometry, request, _) => geometry is Surface surface && request.Parameter is int density && density > 0 && (surface.Domain(0), surface.Domain(1)) is (Interval uDomain, Interval vDomain)
            ...
            [((ExtractionConfig.OpType)10, typeof(Brep))] = static (geometry, _, _) => geometry is Brep
                ? ResultFactory.Create<Point3d[]>(error: E.Geometry.InvalidExtraction.WithContext("DivideByCount unsupported for Brep"))
                : ResultFactory.Create<Point3d[]>(error: E.Geometry.InvalidExtraction.WithContext("Expected Brep")),
            [((ExtractionConfig.OpType)11, typeof(Curve))] = static (geometry, request, _) => geometry is Curve curve && request.Parameter is double length && length > 0
            ...
            [((ExtractionConfig.OpType)11, typeof(Surface))] = static (geometry, _, _) => geometry is Surface
                ? ResultFactory.Create<Point3d[]>(error: E.Geometry.InvalidExtraction.WithContext("DivideByLength unsupported for Surface"))
                : ResultFactory.Create<Point3d[]>(error: E.Geometry.InvalidExtraction.WithContext("Expected Surface")),
            [((ExtractionConfig.OpType)12, typeof(Curve))] = static (geometry, request, _) => geometry is Curve curve && request.Parameter is Vector3d direction
            ...
            [((ExtractionConfig.OpType)13, typeof(Curve))] = static (geometry, request, _) => geometry is Curve curve && request.Parameter is Continuity continuity
            ...
            [((ExtractionConfig.OpType)8, typeof(Curve))] = static (geometry, request, _) => geometry is Curve curve
        ...
        FrozenDictionary<ExtractionConfig.OpType, (Type GeometryType, Func<GeometryBase, Extract.Request, IGeometryContext, Result<Point3d[]>> Handler)[]> fallbacks = map
            .GroupBy(static entry => entry.Key.Operation)
            .ToDictionary(
    ...
    [Pure]
    private static (FrozenDictionary<(ExtractionConfig.OpType Operation, Type GeometryType), Func<GeometryBase, Extract.Request, IGeometryContext, Result<Curve[]>>> Map,
        FrozenDictionary<ExtractionConfig.OpType, (Type GeometryType, Func<GeometryBase, Extract.Request, IGeometryContext, Result<Curve[]>> Handler)[]> Fallbacks) BuildCurveHandlerRegistry() {
        Dictionary<(ExtractionConfig.OpType Operation, Type GeometryType), Func<GeometryBase, Extract.Request, IGeometryContext, Result<Curve[]>>> map = new() {
            [((ExtractionConfig.OpType)20, typeof(Surface))] = static (geometry, _, _) => geometry is Surface surface && (surface.Domain(0), surface.Domain(1)) is (Interval u, Interval v) && new Curve?[] { surface.IsoCurve(direction: 0, constantParameter: u.Min), surface.IsoCurve(direction: 1, constantParameter: v.Min), surface.IsoCurve(direction: 0, constantParameter: u.Max), surface.IsoCurve(direction: 1, constantParameter: v.Max), } is Curve?[] curves
            ...
            [((ExtractionConfig.OpType)20, typeof(Brep))] = static (geometry, _, _) => geometry is Brep brep
            ...
            [((ExtractionConfig.OpType)21, typeof(Surface))] = static (geometry, _, _) => geometry is Surface surface && surface.Domain(1) is Interval vDomain
                ...
                : ResultFactory.Create<Curve[]>(error: E.Geometry.InvalidExtraction.WithContext("Surface domain unavailable")),
            [((ExtractionConfig.OpType)22, typeof(Surface))] = static (geometry, _, _) => geometry is Surface surface && surface.Domain(0) is Interval uDomain
                ...
                : ResultFactory.Create<Curve[]>(error: E.Geometry.InvalidExtraction.WithContext("Surface domain unavailable")),
            [((ExtractionConfig.OpType)23, typeof(Surface))] = static (geometry, _, _) => geometry is Surface surface && (surface.Domain(0), surface.Domain(1)) is (Interval uDomain, Interval vDomain)
                ...
                : ResultFactory.Create<Curve[]>(error: E.Geometry.InvalidExtraction.WithContext("Surface domain unavailable")),
            [((ExtractionConfig.OpType)24, typeof(Brep))] = static (geometry, _, _) => geometry is Brep brep
                ...
                : ResultFactory.Create<Curve[]>(error: E.Geometry.InvalidExtraction.WithContext("Expected Brep")),
            [((ExtractionConfig.OpType)30, typeof(Surface))] = static (geometry, request, _) => geometry is Surface surface && request.Parameter is int count && count >= ExtractionConfig.MinIsocurveCount && count <= ExtractionConfig.MaxIsocurveCount && (surface.Domain(0), surface.Domain(1)) is (Interval uDomain, Interval vDomain)
            ...
            [((ExtractionConfig.OpType)31, typeof(Surface))] = static (geometry, request, _) => geometry is Surface surface && request.Parameter is (int count, byte direction) && count >= ExtractionConfig.MinIsocurveCount && count <= ExtractionConfig.MaxIsocurveCount
                ? direction switch {
            ...
            [((ExtractionConfig.OpType)32, typeof(Surface))] = static (geometry, request, _) => geometry is Surface surface && request.Parameter is double[] parameters && parameters.Length > 0 && (surface.Domain(0), surface.Domain(1)) is (Interval uDomain, Interval vDomain)
            ...
            [((ExtractionConfig.OpType)33, typeof(Surface))] = static (geometry, request, _) => geometry is Surface surface && request.Parameter is (double[] parameters, byte direction) && parameters.Length > 0
                ? direction switch {
                ...
                : ResultFactory.Create<Curve[]>(error: E.Geometry.InvalidExtraction.WithContext("Invalid parameters")),
            [((ExtractionConfig.OpType)34, typeof(Brep))] = static (geometry, request, _) => geometry is Brep brep && request.Parameter is double angleThreshold
        ...
        FrozenDictionary<ExtractionConfig.OpType, (Type GeometryType, Func<GeometryBase, Extract.Request, IGeometryContext, Result<Curve[]>> Handler)[]> fallbacks = map
            .GroupBy(static entry => entry.Key.Operation)
            .ToDictionary(
                static group => group.Key,
