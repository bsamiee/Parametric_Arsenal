    ...
    private static FrozenDictionary<(byte Kind, Type GeometryType), Func<GeometryBase, Extract.Request, IGeometryContext, Result<Curve[]>>> BuildCurveHandlerRegistry() =>
        new Dictionary<(byte, Type), Func<GeometryBase, Extract.Request, IGeometryContext, Result<Curve[]>>> {
            [(20, typeof(Surface))] = static (geometry, _, _) => geometry is Surface surface && (surface.Domain(0), surface.Domain(1)) is (Interval u, Interval v)
                ? ResultFactory.Create<Curve[]>(value: [.. new Curve?[] { surface.IsoCurve(direction: 0, constantParameter: u.Min), surface.IsoCurve(direction: 1, constantParameter: v.Min), surface.IsoCurve(direction: 0, constantParameter: u.Max), surface.IsoCurve(direction: 1, constantParameter: v.Max), }.OfType<Curve>(),])
            ...
            [(21, typeof(Surface))] = static (geometry, _, _) => geometry is Surface surface && surface.Domain(1) is Interval vDomain
                ? ResultFactory.Create<Curve[]>(value: [.. Enumerable.Range(0, 5).Select(i => surface.IsoCurve(direction: 0, constantParameter: vDomain.ParameterAt(i / 4.0))).OfType<Curve>(),])
                : ResultFactory.Create<Curve[]>(error: E.Geometry.InvalidExtraction.WithContext("Surface domain unavailable")),
            [(22, typeof(Surface))] = static (geometry, _, _) => geometry is Surface surface && surface.Domain(0) is Interval uDomain
                ? ResultFactory.Create<Curve[]>(value: [.. Enumerable.Range(0, 5).Select(i => surface.IsoCurve(direction: 1, constantParameter: uDomain.ParameterAt(i / 4.0))).OfType<Curve>(),])
                : ResultFactory.Create<Curve[]>(error: E.Geometry.InvalidExtraction.WithContext("Surface domain unavailable")),
            [(23, typeof(Surface))] = static (geometry, _, _) => geometry is Surface surface && (surface.Domain(0), surface.Domain(1)) is (Interval uDomain, Interval vDomain)
                ? ResultFactory.Create<Curve[]>(value: [.. Enumerable.Range(0, 5).Select(i => surface.IsoCurve(direction: 0, constantParameter: vDomain.ParameterAt(i / 4.0))).OfType<Curve>(), .. Enumerable.Range(0, 5).Select(i => surface.IsoCurve(direction: 1, constantParameter: uDomain.ParameterAt(i / 4.0))).OfType<Curve>(),])
                : ResultFactory.Create<Curve[]>(error: E.Geometry.InvalidExtraction.WithContext("Surface domain unavailable")),
            [(24, typeof(Brep))] = static (geometry, _, _) => geometry is Brep brep
                ? ResultFactory.Create<Curve[]>(value: brep.Edges.Where(edge => edge.AdjacentFaces() is int[] adjacentFaces && adjacentFaces.Length == 2 && edge.PointAt(edge.Domain.ParameterAt(0.5)) is Point3d midPoint && brep.Faces[adjacentFaces[0]].ClosestPoint(testPoint: midPoint, u: out double u0, v: out double v0) && brep.Faces[adjacentFaces[1]].ClosestPoint(testPoint: midPoint, u: out double u1, v: out double v1) && Math.Abs(Vector3d.VectorAngle(brep.Faces[adjacentFaces[0]].NormalAt(u: u0, v: v0), brep.Faces[adjacentFaces[1]].NormalAt(u: u1, v: v1))) >= ExtractionConfig.FeatureEdgeAngleThreshold).Select(edge => edge.DuplicateCurve()).OfType<Curve>().ToArray())
                : ResultFactory.Create<Curve[]>(error: E.Geometry.InvalidExtraction.WithContext("Expected Brep")),
            ...
            [(34, typeof(Brep))] = static (geometry, request, _) => geometry is Brep brep && request.Parameter is double angleThreshold
                ? ResultFactory.Create<Curve[]>(value: brep.Edges.Where(edge => edge.AdjacentFaces() is int[] adjacentFaces && adjacentFaces.Length == 2 && edge.PointAt(edge.Domain.ParameterAt(0.5)) is Point3d midPoint && brep.Faces[adjacentFaces[0]].ClosestPoint(testPoint: midPoint, u: out double u0, v: out double v0) && brep.Faces[adjacentFaces[1]].ClosestPoint(testPoint: midPoint, u: out double u1, v: out double v1) && Math.Abs(Vector3d.VectorAngle(brep.Faces[adjacentFaces[0]].NormalAt(u: u0, v: v0), brep.Faces[adjacentFaces[1]].NormalAt(u: u1, v: v1))) >= angleThreshold).Select(edge => edge.DuplicateCurve()).OfType<Curve>().ToArray())
