...
internal static class MorphologyConfig {
    ...
    /// <summary>Operation metadata keyed by request type and geometry type.</summary>
    internal static readonly FrozenDictionary<(Type RequestType, Type GeometryType), OperationMetadata> Operations =
        new Dictionary<(Type, Type), OperationMetadata> {
            ...
            [(typeof(Morphology.MeshReductionRequest), typeof(Mesh))] = new OperationMetadata(V.Standard | V.MeshSpecific | V.Topology, "MeshReduce"),
    ...
    [Pure]
    internal static bool TryGetMetadata(Type requestType, Type geometryType, out OperationMetadata metadata) =>
        Operations.TryGetValue((requestType, geometryType), out metadata);

    /// <summary>Subdivision algorithm identifiers.</summary>
    internal const byte AlgorithmSubdivideCatmullClark = 2;
    internal const byte AlgorithmSubdivideLoop = 3;
    internal const byte AlgorithmSubdivideButterfly = 4;
    ...
    /// <summary>Mesh repair operation dispatch removed in favor of algebraic operations.</summary>
}
