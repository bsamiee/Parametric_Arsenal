...
internal static class MorphologyCompute {
    ...
    internal static Result<Mesh> SubdivideIterative(
        Mesh mesh,
        MorphologyConfig.SubdivisionScheme algorithm,
        int levels,
                ...
                : Enumerable.Range(0, levels).Aggregate(
                    ...
                    (result, level) => result.Bind(current => {
                        Mesh? next = algorithm switch {
                            MorphologyConfig.SubdivisionScheme.CatmullClark => current.DuplicateMesh(),
                            MorphologyConfig.SubdivisionScheme.Loop => SubdivideLoop(current),
                            MorphologyConfig.SubdivisionScheme.Butterfly => SubdivideButterfly(current),
                            _ => null,
                        };
