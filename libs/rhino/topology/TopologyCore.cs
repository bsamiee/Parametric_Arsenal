...
internal static class TopologyCore {
    ...
    internal static Result<Topology.BoundaryLoopData> ExecuteBoundaryLoops<T>(T input, IGeometryContext context, double? tolerance, bool enableDiagnostics) where T : notnull {
        ...
        return Execute(input: input, context: context, opType: TopologyConfig.OpType.BoundaryLoops, enableDiagnostics: enableDiagnostics,
            operation: g => (g switch {
                Brep brep => Enumerable.Range(0, brep.Edges.Count).Where(i => brep.Edges[i].Valence == EdgeAdjacency.Naked).Select(i => brep.Edges[i].DuplicateCurve()).ToArray(),
                Mesh mesh => (mesh.GetNakedEdges() ?? []).Select(pl => pl.ToNurbsCurve()).ToArray(),
                _ => null,
            }) switch {
                null => ResultFactory.Create<IReadOnlyList<Topology.BoundaryLoopData>>(error: E.Geometry.UnsupportedAnalysis.WithContext($"Type: {typeof(T).Name}")),
    ...
    internal static Result<Topology.ConnectivityData> ExecuteConnectivity<T>(T input, IGeometryContext context, bool enableDiagnostics) where T : notnull =>
        Execute(input: input, context: context, opType: TopologyConfig.OpType.Connectivity, enableDiagnostics: enableDiagnostics,
            operation: g => g switch {
                Brep brep => ComputeConnectivity(faceCount: brep.Faces.Count, getAdjacent: fIdx => brep.Faces[fIdx].AdjacentEdges().SelectMany(eIdx => brep.Edges[eIdx].AdjacentFaces()), getBounds: fIdx => brep.Faces[fIdx].GetBoundingBox(accurate: false), getAdjacentForGraph: fIdx => [.. brep.Faces[fIdx].AdjacentEdges().SelectMany(eIdx => brep.Edges[eIdx].AdjacentFaces()).Where(adj => adj != fIdx),]),
                Mesh mesh => ComputeConnectivity(faceCount: mesh.Faces.Count, getAdjacent: fIdx => mesh.Faces.AdjacentFaces(fIdx).Where(adj => adj >= 0), getBounds: fIdx => mesh.Faces[fIdx] switch { MeshFace face => face.IsQuad ? new BoundingBox([mesh.Vertices[face.A], mesh.Vertices[face.B], mesh.Vertices[face.C], mesh.Vertices[face.D],]) : new BoundingBox([mesh.Vertices[face.A], mesh.Vertices[face.B], mesh.Vertices[face.C],]) }, getAdjacentForGraph: fIdx => [.. mesh.Faces.AdjacentFaces(fIdx).Where(adj => adj >= 0),]),
    ...
    internal static Result<Topology.AdjacencyData> ExecuteAdjacency<T>(T input, IGeometryContext context, int edgeIndex, bool enableDiagnostics) where T : notnull =>
        Execute(input: input, context: context, opType: TopologyConfig.OpType.Adjacency, enableDiagnostics: enableDiagnostics,
            operation: g => (g, edgeIndex) switch {
                ...
                (Brep brep, int idx) => ResultFactory.Create<IReadOnlyList<Topology.AdjacencyData>>(error: TopologyConfig.IndexError(E.Geometry.InvalidEdgeIndex, "EdgeIndex", idx, brep.Edges.Count - 1)),
                ...
                (Mesh mesh, int idx) => ResultFactory.Create<IReadOnlyList<Topology.AdjacencyData>>(error: TopologyConfig.IndexError(E.Geometry.InvalidEdgeIndex, "EdgeIndex", idx, mesh.TopologyEdges.Count - 1)),
    ...
    [Pure]
    private static Result<IReadOnlyList<Topology.ConnectivityData>> ComputeConnectivity(
        int faceCount,
        ...
        for (int seed = 0; seed < faceCount; seed++) {
            componentCount = componentIds[seed] != -1 ? componentCount : (componentCount, new Func<int>(() => {
                Queue<int> queue = new([seed,]);
                componentIds[seed] = componentCount;
                while (queue.Count > 0) {
                    foreach (int adjFace in getAdjacent(queue.Dequeue()).Where(f => componentIds[f] == -1)) {
        ...
        return ResultFactory.Create(value: (IReadOnlyList<Topology.ConnectivityData>)[new Topology.ConnectivityData(ComponentIndices: components, ComponentSizes: [.. components.Select(c => c.Count),], ComponentBounds: [.. components.Select(c => c.Aggregate(BoundingBox.Empty, (u, f) => getBounds(f) is BoundingBox box && box.IsValid ? (u.IsValid ? BoundingBox.Union(u, box) : box) : u)),], TotalComponents: componentCount, IsFullyConnected: componentCount == 1, AdjacencyGraph: Enumerable.Range(0, faceCount).Select(f => (f, getAdjacentForGraph(f))).ToFrozenDictionary(x => x.f, x => x.Item2)),]);
    }
    ...
    private static Result<IReadOnlyList<Topology.EdgeClassificationData>> ClassifyBrepEdges(Brep brep, Continuity minContinuity, double angleThreshold) =>
        Enumerable.Range(0, brep.Edges.Count).Select(i => (
            Index: i,
            Type: brep.Edges[i].Valence switch {
            ...
            Measure: brep.Edges[i].GetLength()
        )).ToArray() switch {
            (int Index, Topology.EdgeContinuityType Type, double Measure)[] data => ResultFactory.Create(value: (IReadOnlyList<Topology.EdgeClassificationData>)[new Topology.EdgeClassificationData(EdgeIndices: [.. data.Select(d => d.Index),], Classifications: [.. data.Select(d => d.Type),], ContinuityMeasures: [.. data.Select(d => d.Measure),], GroupedByType: data.GroupBy(d => d.Type, d => d.Index).ToFrozenDictionary(g => g.Key, g => (IReadOnlyList<int>)[.. g,]), MinimumContinuity: minContinuity),]),
        };

    [Pure]
    private static Result<IReadOnlyList<Topology.EdgeClassificationData>> ClassifyMeshEdges(Mesh mesh, double angleThreshold) =>
        Enumerable.Range(0, mesh.TopologyEdges.Count).Select(i => (
            Index: i,
            Type: mesh.TopologyEdges.GetConnectedFaces(i) switch {
                ...
                int[] cf when cf.Length == 2 => (mesh.FaceNormals[cf[0]].IsValid && mesh.FaceNormals[cf[1]].IsValid ? Vector3d.VectorAngle(mesh.FaceNormals[cf[0]], mesh.FaceNormals[cf[1]]) : Math.PI) switch {
                    double angle when Math.Abs(angle) < angleThreshold * TopologyConfig.CurvatureThresholdRatio => Topology.EdgeContinuityType.Curvature,
                    double angle when Math.Abs(angle) < angleThreshold => Topology.EdgeContinuityType.Smooth,
                    _ => Topology.EdgeContinuityType.Sharp,
                },
                _ => Topology.EdgeContinuityType.Sharp,
            },
            Measure: mesh.TopologyEdges.GetTopologyVertices(i) switch { IndexPair verts => mesh.TopologyVertices[verts.I].DistanceTo(mesh.TopologyVertices[verts.J]) }
        )).ToArray() switch {
            (int Index, Topology.EdgeContinuityType Type, double Measure)[] data => ResultFactory.Create(value: (IReadOnlyList<Topology.EdgeClassificationData>)[new Topology.EdgeClassificationData(EdgeIndices: [.. data.Select(d => d.Index),], Classifications: [.. data.Select(d => d.Type),], ContinuityMeasures: [.. data.Select(d => d.Measure),], GroupedByType: data.GroupBy(d => d.Type, d => d.Index).ToFrozenDictionary(g => g.Key, g => (IReadOnlyList<int>)[.. g,]), MinimumContinuity: Continuity.C0_continuous),]),
        };
    ...
    internal static Result<Topology.VertexData> ExecuteVertexData<T>(T input, IGeometryContext context, int vertexIndex, bool enableDiagnostics) where T : notnull =>
        Execute(input: input, context: context, opType: TopologyConfig.OpType.VertexData, enableDiagnostics: enableDiagnostics,
            operation: g => (g, vertexIndex) switch {
                ...
                (Brep brep, int idx) => ResultFactory.Create<IReadOnlyList<Topology.VertexData>>(error: TopologyConfig.IndexError(E.Geometry.InvalidVertexIndex, "VertexIndex", idx, brep.Vertices.Count - 1)),
                (Mesh mesh, int idx) when idx >= 0 && idx < mesh.TopologyVertices.Count => Enumerable.Range(0, mesh.TopologyEdges.Count).Where(e => mesh.TopologyEdges.GetTopologyVertices(e) is IndexPair p && (p.I == idx || p.J == idx)).ToArray() switch {
                    int[] edges => ResultFactory.Create(value: (IReadOnlyList<Topology.VertexData>)[new Topology.VertexData(VertexIndex: idx, Location: mesh.TopologyVertices[idx], ConnectedEdgeIndices: [.. edges,], ConnectedFaceIndices: [.. mesh.TopologyVertices.ConnectedFaces(idx),], Valence: mesh.TopologyVertices.ConnectedTopologyVertices(idx).Length, IsBoundary: edges.Any(e => mesh.TopologyEdges.GetConnectedFaces(e).Length == 1), IsManifold: edges.All(e => mesh.TopologyEdges.GetConnectedFaces(e).Length == 2)),]),
                },
                (Mesh mesh, int idx) => ResultFactory.Create<IReadOnlyList<Topology.VertexData>>(error: TopologyConfig.IndexError(E.Geometry.InvalidVertexIndex, "VertexIndex", idx, mesh.TopologyVertices.Count - 1)),
    ...
    internal static Result<Topology.NgonTopologyData> ExecuteNgonTopology<T>(T input, IGeometryContext context, bool enableDiagnostics) where T : notnull =>
        Execute(input: input, context: context, opType: TopologyConfig.OpType.NgonTopology, enableDiagnostics: enableDiagnostics,
            operation: g => g switch {
                ...
                Mesh mesh => Enumerable.Range(0, mesh.Ngons.Count).Select(i => (
                    Index: i,
#pragma warning disable IDE0004
                    Faces: (IReadOnlyList<int>)[.. Enumerable.Range(0, (int)mesh.Ngons[i].FaceCount).Select(f => (int)mesh.Ngons[i].FaceIndexList()[f]),],
#pragma warning restore IDE0004
                    Boundary: (IReadOnlyList<int>)[.. mesh.Ngons.GetNgonBoundary([i]) ?? [],],
                    Center: mesh.Ngons.GetNgonCenter(i) switch { Point3d pt when pt.IsValid => pt, _ => Point3d.Origin }
                )).ToArray() switch {
                    (int Index, IReadOnlyList<int> Faces, IReadOnlyList<int> Boundary, Point3d Center)[] data => ResultFactory.Create(value: (IReadOnlyList<Topology.NgonTopologyData>)[new Topology.NgonTopologyData(NgonIndices: [.. data.Select(d => d.Index),], FaceIndicesPerNgon: [.. data.Select(d => d.Faces),], BoundaryEdgesPerNgon: [.. data.Select(d => d.Boundary),], NgonCenters: [.. data.Select(d => d.Center),], EdgeCountPerNgon: [.. data.Select(d => d.Boundary.Count),], TotalNgons: mesh.Ngons.Count, TotalFaces: mesh.Faces.Count),]),
                },
