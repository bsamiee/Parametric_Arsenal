                        ...
                        bool success = currentStrategy switch {
                            ...
                            TopologyConfig.StrategyTargetedJoin => ((Func<bool>)(() => {
                                ...
                                for (int iteration = 0; iteration < TopologyConfig.MaxEdgesForNearMissAnalysis; iteration++) {
                                    ...
                                    for (int i = 0; i < nakedEdgeIndices.Length && !joinedThisPass; i++) {
                                        int edgeIndexA = nakedEdgeIndices[i];
                                        if (edgeIndexA >= copy.Edges.Count) {
                                            continue;
                                        }
                                        BrepEdge edgeA = copy.Edges[edgeIndexA];
                                        if (edgeA.Valence != EdgeAdjacency.Naked) {
                                            continue;
                                        }
                                        for (int j = i + 1; j < nakedEdgeIndices.Length && !joinedThisPass; j++) {
                                            int edgeIndexB = nakedEdgeIndices[j];
                                            if (edgeIndexB >= copy.Edges.Count) {
                                                continue;
                                            }
                                            BrepEdge edgeB = copy.Edges[edgeIndexB];
                                            if (edgeB.Valence != EdgeAdjacency.Naked) {
                                                continue;
                                            }
                                            double distSS = edgeA.PointAtStart.DistanceTo(edgeB.PointAtStart);
                                            double distSE = edgeA.PointAtStart.DistanceTo(edgeB.PointAtEnd);
    ...
    internal static Result<(int Genus, (int LoopIndex, bool IsHole)[] Loops, bool IsSolid, int HandleCount)> ExtractFeatures(
        ...
        !brep.IsValidTopology(out string _)
            ...
            : ResultFactory.Create(value: brep)
                ...
                .Map(validBrep => (V: validBrep.Vertices.Count, E: validBrep.Edges.Count, F: validBrep.Faces.Count, Solid: validBrep.IsSolid && validBrep.IsManifold, Loops: [.. validBrep.Loops.Select((l, i) => { using Curve? loopCurve = l.To3dCurve(); return (LoopIndex: i, IsHole: l.LoopType == BrepLoopType.Inner && (loopCurve?.GetLength() ?? 0.0) > Math.Max(context.AbsoluteTolerance, TopologyConfig.MinLoopLength)); }),]))
                .Bind(data => data switch {
                    (int v, int e, int f, bool solid, (int LoopIndex, bool IsHole)[] loops) when v > 0 && e > 0 && f > 0 && solid && (e - v - f + 2) / 2 is int genus && genus >= 0 => ResultFactory.Create(value: (Genus: genus, Loops: loops, IsSolid: solid, HandleCount: genus)),
                    (int v, int e, int f, bool solid, (int LoopIndex, bool IsHole)[] loops) when v > 0 && e > 0 && f > 0 => ResultFactory.Create(value: (Genus: 0, Loops: loops, IsSolid: solid, HandleCount: 0)),
