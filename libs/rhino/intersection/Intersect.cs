...
#pragma warning restore IDISP007
            ...
            static Result<IntersectionOutput> fromPolylines(Polyline[]? sections) =>
                sections switch { { Length: > 0 } => ResultFactory.Create(value: new IntersectionOutput(
                                      ...
                                      [], [], [], [],
                                      [.. sections])),
                    null => ResultFactory.Create<IntersectionOutput>(error: Arsenal.Core.Errors.E.Geometry.IntersectionFailed),
                    _ => ResultFactory.Create(value: IntersectionOutput.Empty),
                };
            ...
            return (a, b, opts) switch {
                (Point3d[] pts, Brep[] breps, { ProjectionDirection: Vector3d dir }) when !dir.IsValid || dir.Length <= RhinoMath.ZeroTolerance =>
                    ResultFactory.Create<IntersectionOutput>(error: Arsenal.Core.Errors.E.Geometry.InvalidProjection),
                ...
                (Ray3d ray, GeometryBase[] geoms, { MaxHits: int hits }) when ray.Direction.Length <= RhinoMath.ZeroTolerance =>
                    ResultFactory.Create<IntersectionOutput>(error: Arsenal.Core.Errors.E.Geometry.InvalidRay),
                (Ray3d ray, GeometryBase[] geoms, { MaxHits: int hits }) when hits <= 0 =>
                    ResultFactory.Create<IntersectionOutput>(error: Arsenal.Core.Errors.E.Geometry.InvalidMaxHits),
...
#pragma warning restore IDISP004
                ...
                (Mesh ma, Line lb, { Sorted: true }) =>
                    RhinoIntersect.MeshLineSorted(ma, lb, out int[] ids3) switch {
                        ...
                        _ => ResultFactory.Create<IntersectionOutput>(error: Arsenal.Core.Errors.E.Geometry.IntersectionFailed),
                    },
                ...
                _ => ResultFactory.Create<IntersectionOutput>(error: Arsenal.Core.Errors.E.Geometry.UnsupportedIntersection),
            };
        }
    }
}
