...
internal static class IntersectionCore {
    ...
    internal static Result<(IntersectionStrategy Strategy, bool Swapped)> ResolveStrategy(Type typeA, Type typeB) {
        ...
        Type[] chainA = [.. chainAList
            .Concat(chainAList.SelectMany(static baseType => baseType.GetInterfaces()))
            .Distinct()];
        Type[] chainB = [.. chainBList
            .Concat(chainBList.SelectMany(static baseType => baseType.GetInterfaces()))
            .Distinct()];

        ((Type, Type) Key, bool Swapped, bool Found, IntersectionStrategy Strategy) chosen =
            chainA.SelectMany((Type a) => chainB.Select((Type b) => ((a, b), false))).Concat(
                chainB.SelectMany((Type b) => chainA.Select((Type a) => ((b, a), true))))
            .Select(((Type, Type) Key, bool Swapped) candidate =>
                _strategies.TryGetValue(candidate.Key, out IntersectionStrategy resolved)
                    ? (candidate.Key, candidate.Swapped, true, resolved)
                    : (candidate.Key, candidate.Swapped, false, default(IntersectionStrategy)))
    ...
    internal static Result<Intersect.IntersectionOutput> ExecuteWithOptions(object geometryA, object geometryB, IGeometryContext context, (double Tolerance, Intersect.IntersectionOptions Options) normalized) =>
        ResolveStrategy(geometryA.GetType(), geometryB.GetType())
            .Bind(entry => {
                V modeA = entry.Swapped ? entry.Strategy.ModeB : entry.Strategy.ModeA;
                V modeB = entry.Swapped ? entry.Strategy.ModeA : entry.Strategy.ModeB;
