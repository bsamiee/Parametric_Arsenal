...
public static class Spatial {
    ...
    private static readonly FrozenDictionary<(Type Input, Type Query), (ValidationConfig Mode, int BufferSize)> _algorithmConfig =
        new Dictionary<(Type, Type), (ValidationConfig, int)> {
            [(typeof(Point3d[]), typeof(Sphere))] = (ValidationConfig.None, 2048),
            [(typeof(Point3d[]), typeof(BoundingBox))] = (ValidationConfig.None, 2048),
            [(typeof(Point3d[]), typeof((Point3d[], int)))] = (ValidationConfig.None, 2048),
            [(typeof(Point3d[]), typeof((Point3d[], double)))] = (ValidationConfig.None, 2048),
            [(typeof(PointCloud), typeof(Sphere))] = (ValidationConfig.Degeneracy, 2048),
            [(typeof(PointCloud), typeof(BoundingBox))] = (ValidationConfig.Degeneracy, 2048),
            [(typeof(PointCloud), typeof((Point3d[], int)))] = (ValidationConfig.Degeneracy, 2048),
            [(typeof(PointCloud), typeof((Point3d[], double)))] = (ValidationConfig.Degeneracy, 2048),
            [(typeof(Mesh), typeof(Sphere))] = (ValidationConfig.MeshSpecific, 2048),
            [(typeof(Mesh), typeof(BoundingBox))] = (ValidationConfig.MeshSpecific, 2048),
            [(typeof(ValueTuple<Mesh, Mesh>), typeof(double))] = (ValidationConfig.MeshSpecific, 4096),
            [(typeof(Curve[]), typeof(Sphere))] = (ValidationConfig.Degeneracy, 2048),
            [(typeof(Curve[]), typeof(BoundingBox))] = (ValidationConfig.Degeneracy, 2048),
            [(typeof(Surface[]), typeof(Sphere))] = (ValidationConfig.BoundingBox, 2048),
            [(typeof(Surface[]), typeof(BoundingBox))] = (ValidationConfig.BoundingBox, 2048),
            [(typeof(Brep[]), typeof(Sphere))] = (ValidationConfig.Topology, 2048),
            [(typeof(Brep[]), typeof(BoundingBox))] = (ValidationConfig.Topology, 2048),
        }.ToFrozenDictionary();
    ...
    public static Result<IReadOnlyList<int>> Analyze<TInput, TQuery>(
        ...
        _algorithmConfig.TryGetValue((typeof(TInput), typeof(TQuery)), out (ValidationConfig mode, int bufferSize) config) switch {
    ...
    private static Result<IReadOnlyList<int>> ExecuteAlgorithm<TInput, TQuery>(
        ...
        (input, query) switch {
            ...
            (Point3d[] pts, object q) when q is ValueTuple<Point3d[], int> tuple1 => GetTree(pts).Bind(_ => tuple1.Item2 <= 0
                ? ResultFactory.Create<IReadOnlyList<int>>(error: ErrorRegistry.Geometry.SpatialInvalidK)
            ...
            (Point3d[] pts, object q) when q is ValueTuple<Point3d[], double> tuple2 => GetTree(pts).Bind(_ => tuple2.Item2 <= 0
                ? ResultFactory.Create<IReadOnlyList<int>>(error: ErrorRegistry.Geometry.SpatialInvalidDistance)
    ...
    private static Result<IReadOnlyList<int>> ExecuteKNearestPoints(Point3d[] points, Point3d[] needles, int k) =>
        RTree.Point3dKNeighbors(points, needles, k) switch {
            ...
            null => ResultFactory.Create<IReadOnlyList<int>>(error: ErrorRegistry.Geometry.SpatialProximityFailed),
            _ => ResultFactory.Create<IReadOnlyList<int>>(error: ErrorRegistry.Geometry.SpatialProximityFailed),
        };
    ...
    private static Result<IReadOnlyList<int>> ExecuteOverlapSearch(
        ...
        try {
            ...
            return success switch {
                ...
                false => ResultFactory.Create<IReadOnlyList<int>>(error: ErrorRegistry.Geometry.SpatialProximityFailed),
            };
        } finally {
            ArrayPool<int>.Shared.Return(buffer, clearArray: true);
        }
    ...
    private static Result<RTree> GetTree<T>(T source) where T : notnull =>
        _treeFactories.TryGetValue(typeof(T), out Func<object, RTree>? factory) switch {
            ...
            false => ResultFactory.Create<RTree>(error: ErrorRegistry.Validation.SpatialUnsupportedTypeCombo.WithContext($"Type: {typeof(T).Name}")),
        };
