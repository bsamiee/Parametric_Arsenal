...
internal static class AnalysisCompute {
    ...
    internal static Result<(double SmoothnessScore, double[] CurvatureSamples, (double Parameter, bool IsSharp)[] InflectionPoints, double EnergyMetric)> CurveFairness(Curve curve, IGeometryContext context) =>
        Enumerable.Range(0, AnalysisConfig.CurveFairnessSampleCount).Select(i => curve.Domain.ParameterAt(i / (AnalysisConfig.CurveFairnessSampleCount - 1.0))).Select(t => (t, curve.CurvatureAt(t))).Where(pair => pair.Item2.IsValid).ToArray() is (double, Vector3d)[] samples && samples.Length > 2
            ? (samples.Select(s => s.Item2.Length).ToArray() is double[] curvatures && curvatures.Zip(curvatures.Skip(1), (a, b) => Math.Abs(b - a)).ToArray() is double[] diffs && diffs.Average() is double avgDiff,
               Enumerable.Range(1, samples.Length - 2).Where(i => samples[i - 1].Item2.Length * samples[i + 1].Item2.Length < 0 || (Math.Abs(samples[i].Item2.Length) < context.AbsoluteTolerance && Math.Abs(samples[i - 1].Item2.Length - samples[i + 1].Item2.Length) > AnalysisConfig.InflectionSharpnessThreshold)).Select(i => (samples[i].Item1, Math.Abs(samples[i - 1].Item2.Length - samples[i + 1].Item2.Length) > AnalysisConfig.InflectionSharpnessThreshold)).ToArray() is (double, bool)[] inflections,
    ...
    internal static Result<(double[] AspectRatios, double[] Skewness, double[] Jacobians, int[] ProblematicFaces, (int Warning, int Critical) Counts)> MeshForFEA(Mesh mesh, IGeometryContext context) =>
        Enumerable.Range(0, mesh.Faces.Count).Select(i => mesh.Faces.GetFaceCenter(i) is Point3f center && mesh.Faces.GetFaceVertices(i) is MeshFace face && new[] { mesh.Vertices[face.A], mesh.Vertices[face.B], mesh.Vertices[face.C], face.IsQuad ? mesh.Vertices[face.D] : mesh.Vertices[face.A] }.Select(v => (Point3d)v).ToArray() is Point3d[] verts
            ? (verts.Take(face.IsQuad ? 4 : 3).Select(v => v.DistanceTo(center)).ToArray() is double[] dists ? dists.Max() / (dists.Min() + context.AbsoluteTolerance) : 1.0,
               face.IsQuad ? Math.Abs(((verts[1] - verts[0]).Length + (verts[3] - verts[2]).Length) / ((verts[2] - verts[1]).Length + (verts[0] - verts[3]).Length + context.AbsoluteTolerance) - 1.0) : Math.Abs(Vector3d.CrossProduct(verts[1] - verts[0], verts[2] - verts[0]).Length / ((verts[1] - verts[0]).Length * (verts[2] - verts[0]).Length + context.AbsoluteTolerance) - 0.5) * 2.0,
               face.IsQuad ? verts.Take(4).Zip(verts.Skip(1).Take(3).Append(verts[0]), (a, b) => b - a).ToArray() is Vector3d[] edges ? edges.Zip(edges.Skip(1).Append(edges[0]), (e1, e2) => Vector3d.CrossProduct(e1, e2).Length).Min() / (edges.Select(e => e.Length).Average() * edges.Select(e => e.Length).Average() + context.AbsoluteTolerance) : 1.0 : 1.0)
            : (0.0, 0.0, 0.0)).ToArray() is (double, double, double)[] metrics && metrics.Length > 0
