...
internal static class AnalysisCompute {
    ...
    private static readonly FrozenDictionary<Type, (ValidationConfig Mode, Func<object, IGeometryContext, double?, (double, double)?, int?, Point3d?, int, Result<Analysis.IResult>> Compute)> _strategies =
        new Dictionary<Type, (ValidationConfig, Func<object, IGeometryContext, double?, (double, double)?, int?, Point3d?, int, Result<Analysis.IResult>>)> {
            [typeof(Curve)] = (ValidationConfig.Standard | ValidationConfig.Degeneracy, (g, ctx, t, _, _, _, order) =>
                ResultFactory.Create(value: (Curve)g)
                    .Validate(args: [ctx, ValidationConfig.Standard | ValidationConfig.Degeneracy])
                    .Bind(cv => {
                        ...
                        try {
                            ...
                            return amp is not null && cv.FrameAt(param, out Plane frame)
                                ? ResultFactory.Create(value: (Analysis.IResult)new Analysis.CurveData(
                                    ...
                                    cv.GetLength(),
                                    amp.Centroid))
                                : ResultFactory.Create<Analysis.IResult>(error: ErrorRegistry.Geometry.AnalysisCurveFailed);
                        } finally {
                            pool.Return(buffer, clearArray: true);
                        }
                    })),

            [typeof(NurbsCurve)] = (ValidationConfig.Standard | ValidationConfig.Degeneracy, (g, ctx, t, _, _, _, order) =>
            ...
            [typeof(Surface)] = (ValidationConfig.Standard | ValidationConfig.SurfaceContinuity, (g, ctx, _, uv, _, _, order) =>
                ResultFactory.Create(value: (Surface)g)
                    .Validate(args: [ctx, ValidationConfig.Standard | ValidationConfig.SurfaceContinuity])
                    .Bind(sf => {
                        ...
                        if (!sf.Evaluate(u, v, order, out Point3d _, out Vector3d[] derivs) || amp is null || !sf.FrameAt(u, v, out Plane frame)) {
                            return ResultFactory.Create<Analysis.IResult>(error: ErrorRegistry.Geometry.AnalysisSurfaceFailed);
                        }
                        ...
                        return !double.IsNaN(sc.Gaussian) && !double.IsInfinity(sc.Gaussian)
                            ? ResultFactory.Create(value: (Analysis.IResult)new Analysis.SurfaceData(
                                ...
                                amp.Area,
                                amp.Centroid))
                            : ResultFactory.Create<Analysis.IResult>(error: ErrorRegistry.Geometry.AnalysisSurfaceFailed);
                    })),

            [typeof(NurbsSurface)] = (ValidationConfig.Standard | ValidationConfig.SurfaceContinuity, (g, ctx, _, uv, _, _, order) =>
            ...
            [typeof(Brep)] = (ValidationConfig.Standard | ValidationConfig.Topology | ValidationConfig.MassProperties, (g, ctx, _, uv, faceIdx, testPt, order) =>
                ResultFactory.Create(value: (Brep)g)
                    .Validate(args: [ctx, ValidationConfig.Standard | ValidationConfig.Topology | ValidationConfig.MassProperties])
                    .Bind(brep => {
                        ...
                        if (amp is null || vmp is null || !sf.Evaluate(u, v, order, out Point3d _, out Vector3d[] derivs) || !sf.FrameAt(u, v, out Plane frame) || !brep.ClosestPoint(testPoint, out Point3d cp, out ComponentIndex ci, out double uOut, out double vOut, ctx.AbsoluteTolerance * 100, out Vector3d _)) {
                            return ResultFactory.Create<Analysis.IResult>(error: ErrorRegistry.Geometry.AnalysisBrepFailed);
                        }
                        SurfaceCurvature sc = sf.CurvatureAt(u, v);
                        return !double.IsNaN(sc.Gaussian) && !double.IsInfinity(sc.Gaussian)
                            ? ResultFactory.Create(value: (Analysis.IResult)new Analysis.BrepData(
                                ...
                                vmp.Volume,
                                vmp.Centroid))
                            : ResultFactory.Create<Analysis.IResult>(error: ErrorRegistry.Geometry.AnalysisBrepFailed);
                    })),

            [typeof(Mesh)] = (ValidationConfig.MeshSpecific, (g, ctx, _, _, vertIdx, _, _) =>
                ResultFactory.Create(value: (Mesh)g)
                    .Validate(args: [ctx, ValidationConfig.MeshSpecific])
                    .Bind(mesh => {
                        ...
                        return amp is null || vmp is null
                            ? ResultFactory.Create<Analysis.IResult>(error: ErrorRegistry.Geometry.AnalysisMeshFailed)
    ...
    internal static Result<IReadOnlyList<Analysis.IResult>> Execute(
        ...
        _strategies.TryGetValue(geometry.GetType(), out (ValidationConfig mode, Func<object, IGeometryContext, double?, (double, double)?, int?, Point3d?, int, Result<Analysis.IResult>> compute) strategy)
            ? (enableDiagnostics
            ...
            : ResultFactory.Create<IReadOnlyList<Analysis.IResult>>(error: ErrorRegistry.Geometry.AnalysisUnsupportedGeometry.WithContext(geometry.GetType().Name));
}
