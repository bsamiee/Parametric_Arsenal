...
internal static class AnalysisCompute {
    ...
    private static readonly FrozenDictionary<Type, (ulong Mode, Func<object, IGeometryContext, double?, (double, double)?, int?, Point3d?, int, Result<Analysis.IResult>> Compute)> _strategies =
        new Dictionary<Type, (ulong, Func<object, IGeometryContext, double?, (double, double)?, int?, Point3d?, int, Result<Analysis.IResult>>)> {
            [typeof(Curve)] = (Modes.Standard | Modes.Degeneracy, (g, ctx, t, _, _, _, order) =>
                ResultFactory.Create(value: (Curve)g)
                    .Validate(args: [ctx, Modes.Standard | Modes.Degeneracy])
                    .Bind(cv => {
                        ...
                        try {
                            ...
                            return amp is not null && cv.FrameAt(param, out Plane frame)
                                ? ResultFactory.Create(value: (Analysis.IResult)new Analysis.CurveData(
                                    ...
                                    cv.GetLength(),
                                    amp.Centroid))
                                : ResultFactory.Create<Analysis.IResult>(error: ErrorFactory.Create(code: 2310));
                        } finally {
                            pool.Return(buffer, clearArray: true);
                        }
                    })),

            [typeof(NurbsCurve)] = (Modes.Standard | Modes.Degeneracy, (g, ctx, t, _, _, _, order) =>
            ...
            [typeof(Surface)] = (Modes.Standard | Modes.SurfaceContinuity, (g, ctx, _, uv, _, _, order) =>
                ResultFactory.Create(value: (Surface)g)
                    .Validate(args: [ctx, Modes.Standard | Modes.SurfaceContinuity])
                    .Bind(sf => {
                        ...
                        if (!sf.Evaluate(u, v, order, out Point3d _, out Vector3d[] derivs) || amp is null || !sf.FrameAt(u, v, out Plane frame)) {
                            return ResultFactory.Create<Analysis.IResult>(error: ErrorFactory.Create(code: 2311));
                        }
                        ...
                        return !double.IsNaN(sc.Gaussian) && !double.IsInfinity(sc.Gaussian)
                            ? ResultFactory.Create(value: (Analysis.IResult)new Analysis.SurfaceData(
                                ...
                                amp.Area,
                                amp.Centroid))
                            : ResultFactory.Create<Analysis.IResult>(error: ErrorFactory.Create(code: 2311));
                    })),

            [typeof(NurbsSurface)] = (Modes.Standard | Modes.SurfaceContinuity, (g, ctx, _, uv, _, _, order) =>
            ...
            [typeof(Brep)] = (Modes.Standard | Modes.Topology | Modes.MassProperties, (g, ctx, _, uv, faceIdx, testPt, order) =>
                ResultFactory.Create(value: (Brep)g)
                    .Validate(args: [ctx, Modes.Standard | Modes.Topology | Modes.MassProperties])
                    .Bind(brep => {
                        ...
                        if (amp is null || vmp is null || !sf.Evaluate(u, v, order, out Point3d _, out Vector3d[] derivs) || !sf.FrameAt(u, v, out Plane frame) || !brep.ClosestPoint(testPoint, out Point3d cp, out ComponentIndex ci, out double uOut, out double vOut, ctx.AbsoluteTolerance * 100, out Vector3d _)) {
                            return ResultFactory.Create<Analysis.IResult>(error: ErrorFactory.Create(code: 2312));
                        }
                        SurfaceCurvature sc = sf.CurvatureAt(u, v);
                        return !double.IsNaN(sc.Gaussian) && !double.IsInfinity(sc.Gaussian)
                            ? ResultFactory.Create(value: (Analysis.IResult)new Analysis.BrepData(
                                ...
                                vmp.Volume,
                                vmp.Centroid))
                            : ResultFactory.Create<Analysis.IResult>(error: ErrorFactory.Create(code: 2312));
                    })),

            [typeof(Mesh)] = (Modes.MeshSpecific, (g, ctx, _, _, vertIdx, _, _) =>
                ResultFactory.Create(value: (Mesh)g)
                    .Validate(args: [ctx, Modes.MeshSpecific])
                    .Bind(mesh => {
                        ...
                        return amp is null || vmp is null
                            ? ResultFactory.Create<Analysis.IResult>(error: ErrorFactory.Create(code: 2313))
    ...
    internal static Result<IReadOnlyList<Analysis.IResult>> Execute(
        ...
        _strategies.TryGetValue(geometry.GetType(), out (ulong mode, Func<object, IGeometryContext, double?, (double, double)?, int?, Point3d?, int, Result<Analysis.IResult>> compute) strategy)
            ? (enableDiagnostics
