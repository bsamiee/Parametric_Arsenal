...
internal static class AnalysisCompute {
    ...
    internal static Analysis.CurveData? ComputeCurveData(Curve curve, IGeometryContext context, double? parameter, int derivativeOrder) {
    ...
    [Pure, MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal static Analysis.SurfaceData? ComputeSurfaceData(Surface surface, IGeometryContext _, (double U, double V)? parameter, int derivativeOrder) {
    ...
    [Pure, MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal static Analysis.BrepData? ComputeBrepData(Brep brep, IGeometryContext context, (double U, double V)? parameter, int faceIndex, Point3d? testPoint, int derivativeOrder) {
        ...
        return sf.Evaluate(u, v, derivativeOrder, out Point3d _, out Vector3d[] derivs) && sf.FrameAt(u, v, out Plane frame)
            && brep.ClosestPoint(probe, out Point3d cp, out ComponentIndex ci, out double uOut, out double vOut, context.AbsoluteTolerance * AnalysisConfig.BrepClosestPointToleranceMultiplier, out Vector3d _)
            ? ((Func<Analysis.BrepData?>)(() => {
                ...
                return amp is not null && vmp is not null && RhinoMath.IsValidDouble(sc.Gaussian) && RhinoMath.IsValidDouble(sc.Mean)
                    ? new Analysis.BrepData(
                        ...
                        brep.IsManifold, brep.IsSolid, cp, probe.DistanceTo(cp),
    ...
    [Pure, MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal static Analysis.BrepData? ComputeExtrusionData(Extrusion extrusion, IGeometryContext context, (double U, double V)? parameter, int faceIndex, Point3d? testPoint, int derivativeOrder) =>
        extrusion.ToBrep() is Brep brep
            ? ((Func<Analysis.BrepData?>)(() => {
                using Brep extruded = brep;
                return ComputeBrepData(brep: extruded, context: context, parameter: parameter, faceIndex: faceIndex, testPoint: testPoint, derivativeOrder: derivativeOrder);
            }))()
            : null;

    [Pure, MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal static Analysis.MeshData? ComputeMeshData(Mesh mesh, IGeometryContext context, int vertexIndex) {
    ...
    [Pure, MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal static Analysis.SurfaceQualityResult? ComputeSurfaceQuality(Surface surface, IGeometryContext context) {
    ...
    [Pure, MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal static Analysis.CurveFairnessResult? ComputeCurveFairness(Curve curve, IGeometryContext context) {
    ...
    [Pure, MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal static Analysis.MeshFeaResult? ComputeMeshFea(Mesh mesh, IGeometryContext context) {
