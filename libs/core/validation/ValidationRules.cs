...
public static class ValidationRules {
    ...
    private static readonly FrozenDictionary<V, (string[] Properties, string[] Methods, SystemError Error)> _validationRules =
        new Dictionary<V, (string[], string[], SystemError)> {
            [V.Standard] = (["IsValid"], [], E.Validation.Invalid),
            ...
            [V.MassProperties] = (["IsSolid", "IsClosed"], [], E.Validation.MassPropertiesFailed),
            [V.Topology] = (["IsManifold", "IsClosed", "IsSolid", "IsSurface"], ["IsManifold", "IsPointInside"], E.Validation.TopologyInvalid),
            [V.Degeneracy] = (["IsPeriodic", "IsPolyline"], ["IsShort", "IsSingular", "IsDegenerate", "IsRectangular"], E.Validation.Degenerate),
            ...
            [V.MeshSpecific] = (["IsManifold", "IsClosed", "HasNgons", "HasVertexColors", "HasVertexNormals", "IsTriangleMesh", "IsQuadMesh"], ["IsValidWithLog"], E.Validation.MeshNonManifold),
            [V.SurfaceContinuity] = (["IsPeriodic"], ["IsContinuous"], E.Validation.SurfaceDiscontinuous),
        }.ToFrozenDictionary();
                ...
                .SelectMany(flag => {
                    ...
                    return (IEnumerable<(MemberInfo Member, SystemError Error)>)[
                        .. properties.Select(prop => (
                            Member: _memberCache.GetOrAdd(new CacheKey(runtimeType, V.None, prop, 1),
                                static (key, type) => (type.GetProperty(key.Member!) ?? (MemberInfo)typeof(void)), runtimeType),
                            error)),
                        .. methods.Select(method => (
                            Member: _memberCache.GetOrAdd(new CacheKey(runtimeType, V.None, method, 2),
