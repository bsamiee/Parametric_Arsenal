...
public static class E {
    private static readonly FrozenDictionary<int, string> _m =
        new Dictionary<int, string> {
            ...
            [3931] = "Input filtered",
            [3932] = "Input and query type combination not supported",
    ...
    public static SystemError Get(int code, string? context = null) {
        ErrorDomain domain = GetDomain(code: code);
        string message = _m.TryGetValue(key: code, out string? msg)
            ? msg
            : $"Unknown error code: {code.ToString(provider: CultureInfo.InvariantCulture)}";

        return context switch {
            null => new SystemError(domain: domain, code: code, message: message),
            string ctx => new SystemError(domain: domain, code: code, message: message).WithContext(context: ctx),
        };
    }
    ...
    public static class Validation {
        ...
        public static readonly SystemError UnsupportedOperationType = Get(code: 3930);
        public static readonly SystemError InputFiltered = Get(code: 3931);
        public static readonly SystemError UnsupportedTypeCombo = Get(code: 3932);
    }
