...
public readonly record struct SystemError(byte Domain, int Code, string Message) {
    [Pure] private string DebuggerDisplay => string.Create(CultureInfo.InvariantCulture, $"[{E.GetDomainName(this.Domain)}:{this.Code.ToString(CultureInfo.InvariantCulture)}] {this.Message}");

    [Pure]
    public override string ToString() => $"[{E.GetDomainName(this.Domain)}:{this.Code}] {this.Message}";
