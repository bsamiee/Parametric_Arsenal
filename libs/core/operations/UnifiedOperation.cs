    ...
    public static Result<IReadOnlyList<TOut>> Apply<TIn, TOut>(
        ...
        Result<IReadOnlyList<TOut>> execute(TIn item) {
            ...
            Result<TIn> validated = ResultFactory.Create(value: item)
                .Ensure(predicate: config.InputFilter ?? (_ => true),
                    error: config.ErrorPrefix is null ? E.Validation.InputFiltered : E.Validation.InputFiltered.WithContext(context: config.ErrorPrefix))
