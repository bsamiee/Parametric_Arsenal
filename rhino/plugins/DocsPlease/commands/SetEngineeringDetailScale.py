...
from libs.common_utils import CommonUtils, require_user_choice
from libs.constants import Constants, Strings
from libs.detail_tools import DetailTools
from libs.exceptions import EnvironmentError, ValidationError
...
@rhino_command(requires_layout=True, undo_description="Engineering Detail Scaling")
def set_engineering_detail_scale() -> None:
    """Sets engineering scales on Detail Views with options for individual or batch operations.

    Raises:
        UserCancelledError: If user cancels any selection or input
        EnvironmentError: If unit system is not Imperial
        ValidationError: If no details found or invalid scale
        ScaleError: If scale operations fail
    """
    # Validate environment - engineering scales require Imperial units
    units = CommonUtils.get_model_unit_system()
    if units not in {Rhino.UnitSystem.Inches, Rhino.UnitSystem.Feet}:
        raise EnvironmentError("Engineering scale requires Imperial units (Feet or Inches)")
    ...
    if not page_scale:
        ...
        choice = require_user_choice(keys, Strings.PROMPT_SET_PAGE_SCALE, Strings.PROMPT_ENGINEERING_DETAIL_SCALES)
